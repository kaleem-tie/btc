...
if(! empty($result)){
	...
	label_cell("Topic : <b>".$row["subject"].'</b><br><P>'.htmlspecialchars_decode($row["description"])."</P>");
	...
	if (true) {
		if($row["id"]>0){
			...
			if( $Mode == 'Reply' &&  $selected_id == $row['id']){
					...
					start_row();
					textarea_cells(_("Reply :"),'complaint_reply',null,90,8);
...
if (!$is_closed) {
	...
	textarea_row(_("Reply :"),'reply',null,90,8);
