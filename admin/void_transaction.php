...
function check_valid_entries()
{
	...
	if($_POST['filterType']==ST_CUSTPDC )
	{
		$recall_status = check_voided_pdc_recall_status($_POST['filterType'],$_POST['trans_no']);
		if($recall_status>0)
		{
		 display_error(_("This pdc transaction recalled! once recalled you cannot void the transaction!"));
		set_focus('trans_no');
		return false;
		}
	}
	...
	if($_POST['filterType']==ST_SUPPPDC )
	{
		$recall_status = check_voided_supplier_pdc_recall_status($_POST['filterType'],$_POST['trans_no']);
		if($recall_status>0)
		{
		 display_error(_("This supplier pdc transaction recalled! once recalled you cannot void the transaction!"));
		set_focus('trans_no');
		return false;
		}
	}
	// display_error("ramesh");die;
	return true;
}
