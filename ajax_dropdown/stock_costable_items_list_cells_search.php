...
if(!isset($dataval)){
	...
	$search = $dataval;
	  ...
	  $sql = "SELECT stock_id, CONCAT(stock_id,' ',s.description) as item_desc, c.description as category_description, s.inactive, s.editable FROM ".TB_PREF."stock_master s, ".TB_PREF."stock_category c WHERE s.category_id=c.category_id  AND mb_flag!='D' AND (stock_id LIKE '%".$search."%' or  s.description LIKE '%".$search."%' OR s.supplier_item_code LIKE '%".$search."%') order by c.description, stock_id LIMIT 0,100";
