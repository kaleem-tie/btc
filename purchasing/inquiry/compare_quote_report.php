...
$js = "";
	echo "<center><b>Purchase Quote Comparison: ".$_GET['comparison_code']."</b></center><br>";
?>
<html>
<head>
<style>
...
<?php 
$dec = user_price_dec();
if($_GET['comparison_code'])
{
	if(empty($_GET['comparison_code']) || $_GET['comparison_code']=='')
	{
		display_error("Invalid Code!");
	}
	else
	{
		$comparison_code=$_GET['comparison_code'];
		...
		if($row[0]>0)
		{
		  ...
		  while($row=db_fetch($result))
          {
			...
			?>
			<center>
		<table class="tablestyle2" width="90%" border="1">
		<tr>
		...
		<?php
		$sql="SELECT distinct pod.item_code,pod.description FROM ".TB_PREF."purch_orders po,".TB_PREF."purch_order_details pod WHERE po.order_no=pod.order_no and po.comparison_code='$comparison_code' and pod.trans_type=po.trans_type and po.trans_type=52";
		$result = db_query($sql, "could not get result");
	...
	<input id="printpagebutton" type="button" value="Print" class="no-print" onclick="printpage()" style="background-color:#9ec4c2;"/>	
</center>
		<?php
		}
		else
