...
page(_($help_context = "View Purchase Quotation"), true, false, "", $js);


if (!isset($_GET['trans_no']))
{
	die ("<br>" . _("This page must be called with a purchase quotation number to review."));
}
...
display_heading(_("Purchase Quotation") . " #" . $_GET['trans_no']);

$purchase_order = new purch_order;

read_po($_GET['trans_no'], $purchase_order,false,ST_PURCHQUOTE);
echo "<br>";
display_po_summary($purchase_order, true,false,ST_PURCHQUOTE);
...
$th = array(_("S.No."),_("Item Code"), _("Item Description"), _("Quantity"), _("Unit"), _("Price"),
	_("Net Amount"), _("Discount %"),_("Discount Amount"), _("Line Total"));
table_header($th);
$total = $k = 0;
$overdue_items = false;
$total_qty =0;
...
label_cell("","colspan=6 align='right'");
...
$tax_total = display_edit_tax_items($taxes, 9, $purchase_order->tax_included,2);
...
end_page(true, false, false, ST_PURCHQUOTE, $_GET['trans_no']);
