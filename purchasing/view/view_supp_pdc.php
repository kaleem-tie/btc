...
page(_($help_context = "View PDC Payment to Supplier"), true, false, "", $js);
...
$receipt = get_supp_trans($trans_no, ST_SUPPPDC);
...
display_heading(_("PDC Payment to Supplier") . " #$trans_no");
...
comments_display_row(ST_SUPPPDC, $trans_no);

end_table(1);

$voided = is_voided_display(ST_SUPPPDC, $trans_no, _("This payment has been voided."));

// now display the allocations for this payment
if (!$voided) 
{
	display_allocations_from(PT_SUPPLIER, $receipt['supplier_id'], ST_SUPPPDC, $trans_no, -$receipt['Total']);
}

end_page(true, false, false, ST_SUPPPDC, $trans_no);
