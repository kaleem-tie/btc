...
elseif (isset($_GET['NewGRN'])) {
	...
	if (isset($_GET['FixedAsset'])) {
		...
		$_SESSION['page_title'] = _($help_context = "Direct Purchase Invoice Entry(Against Reverse Charge)");
}
...
if(list_updated('reverse_charge') || list_updated('rc_supplier_id'))
{
	 //$Ajax->activate('last_invoice');
...
function check_data()
{
	...
	if(get_post('rc_supplier_id_rev') == ''){
		
		display_error("Please select line item wise supplier");
		set_focus('rc_supplier_id_rev');
		return false;
	}
	if(get_post('rc_supplier_invoice_id_rev') == ''){
		
		display_error("Please select line item wise purchase reference");
		set_focus('rc_supplier_invoice_id_rev');
		return false;
	}
...
function handle_update_item()
{
	...
	if ($allow_update)
	{
		...
		$_SESSION['PO']->update_order_item($_POST['line_no'], input_num('qty'), input_num('price'),
  			@$_POST['req_del_date'], $_POST['item_description'], input_num('discount_percent'), input_num('disc_amount'),$_POST['line_dimension'],$_POST['rc_supplier_id_rev'],$_POST['rc_supplier_invoice_id_rev'] );
		unset_form_variables();
	}

    unset($_POST['_stock_id_edit'], $_POST['stock_id'],$_POST['line_no'],$_POST['discount_percent'] ,$_POST['disc_amount'],$_POST['line_dimension'],$_POST['rc_supplier_id_rev'],$_POST['rc_supplier_invoice_id_rev']);
...
function handle_add_new_item()
{
		...
		$_POST['disc_amount'] = (input_num('price')*input_num('qty'))*(input_num('discount_percent')/100);
			...
			if ($allow_update)
			{
				$_SESSION['PO']->add_to_order (count($_SESSION['PO']->line_items), $_POST['stock_id'], input_num('qty'), 
					...
					$_SESSION['PO']->trans_type == ST_PURCHORDER ? $_POST['req_del_date'] : '', 0, 0,input_num('discount_percent'),input_num('disc_amount'),$_POST['line_dimension'],$_POST['rc_supplier_id_rev'],$_POST['rc_supplier_invoice_id_rev']);
	...
	unset($_POST['_stock_id_edit'], $_POST['stock_id'],$_POST['discount_percent'], $_POST['disc_amount'],$_POST['line_dimension'],$_POST['rc_supplier_id_rev'],$_POST['rc_supplier_invoice_id_rev']);
...
display_po_header($_SESSION['PO'],1);
echo "<br>";

display_po_reverse_items($_SESSION['PO']);
