...
if (isset($_GET['New']))
{
	...
	if (isset($_GET['invoice_no']))
	{
		...
		$help_context = "Supplier Direct Credit Note";
		$_SESSION['page_title'] = _("Supplier Direct Credit Note");
...
function handle_commit_credit_note()
{
	...
	if (isset($_POST['invoice_no']))
		...
		$invoice_no = add_supp_invoice($_SESSION['supp_trans'],'1');
...
// display_error(json_encode($_POST)); die;
	if (check_item_data($n))
	{
		...
		$_SESSION['supp_trans']->add_grn_to_trans($n,
    		'', $_POST['stock_id'],
    		$item_info['description'], 0,
    		0, input_num('qty'),
    		input_num('price'), input_num('price'), $complete,
			'0', "",input_num('discount_percent'),0,'','');
...
$id = find_submit('EnterLine');

if ($id != -1)
{
	$id = count(($_SESSION['supp_trans']->grn_items))+1;
...
if ($id5 != -1)
{
	...
	//$_SESSION['supp_trans']->remove_grn_from_trans($id5);
	//commit_item_data($id5);
	$Ajax->activate('items_table');
	reset_tax_input();
}
$id6 = find_submit('UpdateLine');
if ($id6 != -1)
{
		$line_no = $_POST['line_no'];
	   $_SESSION['supp_trans']->grn_items[$line_no]->this_quantity_inv = input_num('qty');
		 $_SESSION['supp_trans']->grn_items[$line_no]->order_price = input_num('price');
		 $_SESSION['supp_trans']->grn_items[$line_no]->discount_percent = input_num('discount_percent');
		
	commit_item_data($line_no);
	//$_SESSION['supp_trans']->remove_grn_from_trans($id6);
	//commit_item_data($id6);
	$Ajax->activate('items_table');
	reset_tax_input();
}
...
direct_creditnote_header($_SESSION['supp_trans']);
if ($_POST['supplier_id']=='') 
	display_error('No supplier found for entered search text');
else {
	display_direct_credit_items($_SESSION['supp_trans'], 1);
