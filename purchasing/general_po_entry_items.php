...
***********************************************************************/
$path_to_root = "..";
$page_security = 'SA_GEN_PURCHASEORDER';
include_once($path_to_root . "/purchasing/includes/gen_po_class.inc");
...
if (isset($_GET['ModifyOrderNumber']) && is_numeric($_GET['ModifyOrderNumber'])) {
	...
	$_SESSION['page_title'] = _($help_context = "Modify General Purchase Order #") . $_GET['ModifyOrderNumber'];
	create_new_gen_po(ST_GEN_PURCHORDER, $_GET['ModifyOrderNumber']);
	copy_from_cart();
}
 elseif (isset($_GET['NewOrder'])) {

	$_SESSION['page_title'] = _($help_context = "General Purchase Order Entry");
	create_new_gen_po(ST_GEN_PURCHORDER, 0);
	copy_from_cart();
}
page($_SESSION['page_title'], false, false, "", $js);

if (isset($_GET['ModifyOrderNumber']))
	check_is_editable(ST_GEN_PURCHORDER, $_GET['ModifyOrderNumber']);
...
function check_data()
{
	if(!get_post('item_description', true)) {
    ...
    if ($_SESSION['PO']->trans_type == ST_GEN_PURCHORDER && !is_date($_POST['req_del_date'])){
...
function handle_update_item()
{
	...
	if ($allow_update)
	{
		...
		$_SESSION['PO']->update_order_item($_POST['line_no'],$_POST['stock_id'],input_num('qty'), input_num('price'),
  			@$_POST['req_del_date'], $_POST['item_description'], input_num('discount_percent'), input_num('disc_amount'),0,0,0,$_POST['units']);
		unset_form_variables();
	}
		...
		if ($allow_update == true)
		{
			...
			if ($allow_update)
			{
				...
				$_SESSION['PO']->add_to_order (count($_SESSION['PO']->line_items), $_POST['stock_id'], input_num('qty'), 
					get_post('item_description'), //$myrow["description"], 
					input_num('price'),$_POST['units'], // $myrow["units"], (retrived in cart)
					$_SESSION['PO']->trans_type == ST_GEN_PURCHORDER ? $_POST['req_del_date'] : '', 0, 0,input_num('discount_percent'),input_num('disc_amount'));
...
function can_commit()
{
	if (!get_post('supp_name')) 
	{
		display_error(_("Please enter the Supplier Name."));
		set_focus('supp_name');
		return false;
	} 
	...
	if ($_SESSION['PO']->trans_type == ST_GEN_PURCHORDER && get_post('delivery_address') == '')
	{
...
function handle_commit_order()
{
	...
	if (can_commit()) {
		copy_to_cart_for_gen_po();
		new_doc_date($cart->orig_order_date);
		if ($cart->order_no == 0) { // new po/grn/invoice
			$trans_no = add_gen_direct_supp_trans($cart);
			if ($trans_no) {
				unset($_SESSION['PO']);
				if ($cart->trans_type == ST_GEN_PURCHORDER)
	 				meta_forward($_SERVER['PHP_SELF'], "AddedID=$trans_no");
			}
		} else { // order modification
		      
			$order_no = update_gen_po($cart);
			unset($_SESSION['PO']);
...
display_gen_po_header($_SESSION['PO']);
echo "<br>";
display_gen_po_items($_SESSION['PO']);
