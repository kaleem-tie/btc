...
if (isset($_GET['AddedID']))
{
	...
	display_notification_centered(_("GRN has been processed"));
...
function display_po_receive_items()
{
   ...
   $th = array(_("Item Code"), _("Description"), _("Ordered"), _("Units"), _("Received"),
    	_("Outstanding"), _("This Delivery"), _("Price"), _("Net Amount"),  _("Discount %"),_("Discount Amount"),  _("Total"));
    table_header($th);
    ...
    if (count($_SESSION['PO']->line_items)> 0 )
    {
       	foreach ($_SESSION['PO']->line_items as $ln_itm)
       	{
    		...
    		$line_total = round2($ln_itm->receive_qty * $ln_itm->price * (1 - $ln_itm->discount_percent/100),
	         user_price_dec());
			 
			 $net_amount = round2($ln_itm->receive_qty * $ln_itm->price ,
	         user_price_dec()); 
			 
			 $total += $line_total;
			$data = get_purchase_data($_SESSION['PO']->supplier_id, $ln_itm->stock_id);
			if($data !== false)
			...
			$discount_amount = ($ln_itm->price*$ln_itm->receive_qty)*$ln_itm->discount_percent/100 ;  
			
			amount_cell($net_amount);
			
			amount_cell($display_discount_percent);
	...
	$display_total = price_format(($total + input_num('freight_cost')+ input_num('additional_charges')+ input_num('packing_charges') + input_num('other_charges') + $tax_total));
...
function process_receive_po()
{
	...
	$grn->freight_cost = input_num('freight_cost');
	$grn->additional_charges = input_num('additional_charges');
	$grn->packing_charges = input_num('packing_charges');
	$grn->other_charges = input_num('other_charges');
