	...
	if ($input_error != 1) 
	{
		...
		while (($id = find_submit('tax_type_id'))!=-1)
		{
			if (check_value('tax_type_id'.$id) != 0)
			{
				...
				$tax_additional[] = check_value('tax_additional'.$id);
				$tax_packing[] = check_value('tax_packing'.$id);
				$tax_other[] = check_value('tax_other'.$id);
				
			}	
			unset($_POST['tax_type_id' . $id]);
			unset($_POST['tax_shipping' . $id]);
			
			unset($_POST['tax_additional' . $id]);
			unset($_POST['tax_packing' . $id]);
			unset($_POST['tax_other' . $id]);
		}
    	if ($selected_id != -1) 
    	{
	   		update_tax_group($selected_id, $_POST['name'], $taxes, $tax_shippings, $tax_additional, $tax_packing, $tax_other);
			display_notification(_('Selected tax group has been updated'));
    	} 
    	else 
    	{
	   		add_tax_group($_POST['name'], $taxes, $tax_shippings, $tax_additional, $tax_packing, $tax_other);
...
start_table(TABLESTYLE2);
$th = array(_("Tax"), "", _("Shipping Tax"), _("Additional Tax"), _("Packing Tax"), _("Other Tax"));
table_header($th);
...
while($item = db_fetch($items)) 
{
	...
	if ($selected_id != -1)
	{
		...
		if (isset($item['rate'])){
			...
			check_cells(null, 'tax_additional' . $item['tax_type_id'], $item['tax_additional']);
			
			check_cells(null, 'tax_packing' . $item['tax_type_id'], $item['tax_packing']);
			
			check_cells(null, 'tax_other' . $item['tax_type_id'], $item['tax_other']);
		}
	}
	else
	{
		...
		if (check_value('tax_type_id' . $item['tax_type_id'])==1){
			...
			check_cells(null, 'tax_additional' . $item['tax_type_id'], null);
			
			check_cells(null, 'tax_packing' . $item['tax_type_id'], null);
			
			check_cells(null, 'tax_other' . $item['tax_type_id'], null);
