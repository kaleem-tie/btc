...
if (get_company_pref('use_dimension')) 
	label_row(_("Dimension"), get_user_dimension_name($myrow["dimension_id"]), "class='tableheader2'", "colspan=3");
if (get_company_pref('use_dimension') == 2)
	label_row(_("Dimension"), get_user_dimension_name($myrow["dimension2_id"]), "class='tableheader2'", "colspan=3");
end_row();
...
$preared_user = get_transaction_prepared_by(ST_SALESINVOICE, $trans_id);
label_cells(_("Prepared By"), $preared_user, "class='tableheader2'");	

global $invoice_types;
label_cells(_("Invoice Type"), $invoice_types[$myrow["invoice_type"]], "class='tableheader2'");
end_row();
...
if (db_num_rows($result) > 0)
{
	$th = array( _("S.No."), _("Item Code"), _("Item Description"), _("Quantity"), _("FOC Quantity"),
		_("Unit"), _("Price"), _("Discount %"), _("Discount Amt."), _("Total"));
	table_header($th);
	...
	label_cells(_("Sub-total"), $display_sub_tot, "colspan=9 align=right",
...
$display_total = price_format($myrow["ov_freight"]+$myrow["ov_gst"]+$myrow["ov_amount"]+$myrow["ov_freight_tax"]+$myrow["ov_roundoff"]);

label_row(_("TOTAL INVOICE"), $display_total, "colspan=9 align=right",
	"nowrap align=right");
if ($myrow['prep_amount'])
	label_row(_("PREPAYMENT AMOUNT INVOICED"), '<b>'.price_format($myrow['prep_amount']).'</b>', "colspan=9 align=right",
		"nowrap align=right");
end_table(1);
...
if (!$voided)
{
	display_allocations_to(PT_CUSTOMER, $myrow['debtor_no'], ST_SALESINVOICE, $trans_id, 
	$myrow['Total']);
