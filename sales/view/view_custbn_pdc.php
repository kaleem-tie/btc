...
page(_($help_context = "View Customer Bounce PDC"), true, false, "", $js);
...
$receipt = get_customer_trans($trans_id, ST_CUSTBNPDC);

display_heading(sprintf(_("Customer Bounce PDC #%d"),$trans_id));
...
comments_display_row(ST_CUSTBNPDC, $trans_id);

end_table(1);

$voided = is_voided_display(ST_CUSTBNPDC, $trans_id, _("This customer payment has been voided."));

if (!$voided)
{
	display_allocations_from(PT_CUSTOMER, $receipt['debtor_no'], ST_CUSTBNPDC, $trans_id, $receipt['Total']);
}

end_page(true, false, false, ST_CUSTBNPDC, $trans_id);
