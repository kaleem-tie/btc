...
if (list_updated('BranchID')) {
	...
	$_POST['sales_person_id'] = $br['salesman'];
	
	$Ajax->activate('customer_id');
	$Ajax->activate('sales_person_id');
}
...
if (!isset($_POST['customer_id'])) {
	...
	$result = get_branch_to_order($_POST['customer_id'], $_POST['BranchID']);
	$myrow = db_fetch($result);
	$_POST['sales_person_id'] = $myrow['salesman'];
}
...
function can_process()
{
	...
	if($_POST['bank_account']==1){
		$sales_person_ref_cash_exist = check_exists_cash_sales_person_ref_no(trim($_POST['sales_person_ref']));
	   if($sales_person_ref_cash_exist!=0 && $_POST['trans_no']==0){
		...
		$sales_person_ref_credit_exist = check_exists_credit_sales_person_ref_no(trim($_POST['sales_person_ref']));
	...
	if($_POST['mode_of_payment'] == 'cheque')
	{
		...
		$cheque_no_exist = check_exists_bank_payment_cheque_no(trim($_POST['cheque_no']), $_POST['trans_no'], ST_CUSTPAYMENT);
	   if($cheque_no_exist!=0 && $_POST['trans_no']==0){
		   
		   $cust_cheque_no = get_exists_bank_payment_cheque_no(trim($_POST['cheque_no']),ST_CUSTPAYMENT);
...
if (get_post('AddPaymentItem') && can_process()) {
	...
	$payment_no = write_customer_payment($_SESSION['alloc']->trans_no, $_POST['customer_id'], $_POST['BranchID'],$_POST['bank_account'], $_POST['DateBanked'], 
	$_POST['ref'], input_num('amount'), input_num('discount'), $_POST['memo_'], 0, input_num('charge'), 
	input_num('bank_amount', input_num('amount')), $_POST['dimension_id'], $_POST['dimension2_id'],$_POST['mode_of_payment'],
	$_POST['cheque_no'],$_POST['dd_no'],$_POST['date_of_issue'],$_POST['dd_date_of_issue'],$_POST['pymt_ref'],
	$_POST['our_ref_no'],check_value('amex'),$_POST['sales_person_id'],$_POST['sales_person_ref'],null);
	
	update_recall_info_against_pdc($payment_no,$_POST['DateBanked'],$_POST['PdcNumber'],$_POST['customer_id']);
...
table_section(1);
// PDC Recall ## Ramesh  19/12/2023
if(isset($_GET['PdcNumber']))
{
	$PdcNumber = $_GET['PdcNumber'];
	$receipt = get_customer_trans($PdcNumber, ST_CUSTPDC);
	$_POST['customer_id'] = $receipt['debtor_no'];
...
/*
sales_persons_list_row(_("Sales Person:"), 'sales_person_id',$_POST['sales_person_id'],
false,true);

if (list_updated('sales_person_id')){
	$Ajax->activate('_page_body');
}
...
hidden('sales_person_id', 0);
hidden('sales_person_ref', 0);
...
//text_row(_("Our Ref No.<b style='color:red;'>*</b>"), 'our_ref_no', null, 16, 15);

//text_row(_("Our Ref No."), 'our_ref_no', null, 16, 15); // removed mandatory
...
div_start('alloc_tbl');
show_allocatable(false,$PdcNumber);
div_end();
...
start_row();
	...
	echo '<h5 align="center" style="color:green; font-size: 11px;"><a style="color:green; font-size: 11px;" target="_blank" href="./inquiry/customer_inquiry.php?filterType=3"  >Previous Transactions</a> </h5>';
