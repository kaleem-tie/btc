...
function check_quantities()
{
	...
	foreach ($_SESSION['Items']->line_items as $line=>$itm) {
		...
		if (isset($_POST['price'.$line])) {
			$_SESSION['Items']->line_items[$line]->price =
				  input_num('price'.$line);
...
$th = array(_("S.No."), _("Item Code"), _("Item Description"), 
	$new ? _("Ordered") : _("Max. delivery"),  _("FOC Quantity"), _("Units"), 
...
foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
	...
	$invoicedStatus = isDoInvoiced($_SESSION['Items']->order_no);
	$so_ref = get_inv_sales_order_reference($_SESSION['Items']->order_no);
	
	if($invoicedStatus == 0 && $_SESSION['Items']->reference != 'auto' && $so_ref == 'auto') {
	$_POST['price'.$line] = $ln_itm->price;
	small_amount_cells(null, 'price'.$line, $ln_itm->price, null, null, $dec);
	}else{
