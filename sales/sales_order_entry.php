...
include_once($path_to_root . "/sales/includes/db/customer_unlock_db.inc");
set_page_security( @$_SESSION['Items']->trans_type,
	array(	ST_SALESORDER=>'SA_SALESORDER',
			ST_SALESQUOTE => 'SA_SALESQUOTE',
			ST_SALESENQ => 'SA_SALESENQ',
	...
	array(	'NewOrder' => 'SA_SALESORDER',
			...
			'NewEnqToSalesQuote' => 'SA_SALESENQ',
			'AddedQU' => 'SA_SALESQUOTE',
			'UpdatedQU' => 'SA_SALESQUOTE',
			'NewEnq' => 'SA_SALESENQ',
			'ModifyEnq' => 'SA_SALESENQ',
			'AddedEnq' => 'SA_SALESENQ',
			'UpdatedEnq' => 'SA_SALESENQ',
...
elseif (isset($_GET['ModifyEnq']) && is_numeric($_GET['ModifyEnq'])) {

	$help_context = 'Modifying Sales Enquiry';
	$_SESSION['page_title'] = sprintf( _("Modifying Sales Enquiry # %d"), $_GET['ModifyEnq']);
	create_cart(ST_SALESENQ, $_GET['ModifyEnq']);
...
}elseif (isset($_GET['NewEnq'])) {

	$_SESSION['page_title'] = _($help_context = "New Sales Enquiry Entry");
	create_cart(ST_SALESENQ, 0);
}

//ravi
elseif (isset($_GET['ModifyInvNumber']) && is_numeric($_GET['ModifyInvNumber'])) {
	$help_context = 'Modifying Sales Invoice';
	$_SESSION['page_title'] = sprintf( _("Modifying Sales Invoice # %d"), $_GET['ModifyInvNumber']);
	create_cart(ST_SALESINVOICE, $_GET['ModifyInvNumber']);
}

elseif (isset($_GET['ModifyDelNumber']) && is_numeric($_GET['ModifyDelNumber'])) {
	$help_context = 'Modifying Sales Delivery';
	$_SESSION['page_title'] = sprintf( _("Modifying Sales Delivery # %d"), $_GET['ModifyDelNumber']);
	create_cart(ST_CUSTDELIVERY, $_GET['ModifyDelNumber']);
}
...
elseif (isset($_GET['NewQuoteToSalesOrder'])) {
	...
	$_POST['NewQuoteToSalesOrder']=$_GET['NewQuoteToSalesOrder'];
	$_SESSION['page_title'] = _($help_context = "Sales Order Entry");
	create_cart(ST_INSSALESQUOTE, $_GET['NewQuoteToSalesOrder']);
}
elseif (isset($_GET['NewEnqToSalesQuote'])) {
	$_SESSION['page_title'] = _($help_context = "Sales Quotation Entry");
	create_cart(ST_SALESENQ, $_GET['NewEnqToSalesQuote']);
}
...
elseif (isset($_GET['ModifyEnq']))
    check_is_editable(ST_SALESENQ, $_GET['ModifyEnq']);	

//-----------------------------------------------------------------------------

if (isset($_GET['ModifyInvNumber']))
{
	$_POST['ModifyInvNumber'] = $_GET['ModifyInvNumber'];
}

if (isset($_GET['ModifyDelNumber']))
{
	$_POST['ModifyDelNumber'] = $_GET['ModifyDelNumber'];
}
...
if (isset($_GET['AddedID'])) {
	...
	submenu_option(_("Receive &Payment Against This Order"), "/sales/customer_advance_payments.php?order_no=".$order_no);
...
elseif (isset($_GET['AddedEnq'])) {
	$order_no = $_GET['AddedEnq'];
	display_notification_centered(sprintf( _("Enquiry # %d has been entered."),$order_no));

	submenu_view(_("&View This Enquiry"), ST_SALESENQ, $order_no);

	submenu_print(_("&Print This Enquiry"), ST_SALESENQ, $order_no, 'prtopt');
	submenu_print(_("&Email This Enquiry"), ST_SALESENQ, $order_no, null, 1);
	set_focus('prtopt');
	
	submenu_option(_("Make &Sales Quotation Against This Enquiry"),
		"/sales/sales_order_entry.php?NewEnqToSalesQuote=$order_no");

	submenu_option(_("Enter a New &Enquiry"),	"/sales/sales_order_entry.php?NewEnq=0");

	display_footer_exit();

}
elseif (isset($_GET['UpdatedEnq'])) {
	$order_no = $_GET['UpdatedEnq'];

	display_notification_centered(sprintf( _("Enquiry # %d has been updated."),$order_no));
	...
	submenu_option(_("Select A Different &Enquiry"),
		"/sales/inquiry/sales_orders_view.php?type=".ST_SALESENQ);
...
function copy_to_cart()
{
	...
	$cart->sales_person_id = $_POST['salesman']; //ravi
	...
	$cart->delivery_time = $_POST['delivery_time'];
	$cart->invoice_type  = $_POST['invoice_type'];
	$cart->ov_roundoff = input_num('ov_roundoff');
	
	$cart->lpo_no    = $_POST['lpo_no'];
	$cart->lpo_date  = $_POST['lpo_date'];
	
	$cart->delivery_terms    = $_POST['delivery_terms'];
	
	$cart->cash_cust_name   = $_POST['cash_cust_name'];
	$cart->cash_cust_mobile = $_POST['cash_cust_mobile'];
...
function copy_from_cart()
{
	...
	$_POST['delivery_time'] = $cart->delivery_time;
	$_POST['invoice_type']  = $cart->invoice_type;
	$_POST['ov_roundoff'] = price_format($cart->ov_roundoff);
	
	$_POST['lpo_no']    = $cart->lpo_no;
	$_POST['lpo_date']  = $cart->lpo_date;
	
	$_POST['delivery_terms']    = $cart->delivery_terms;
	
	$_POST['cash_cust_name']   = $cart->cash_cust_name;
	$_POST['cash_cust_mobile'] = $cart->cash_cust_mobile;
...
function can_process() {
	...
	//  display_error(json_encode($_SESSION['Items']));die;
	...
	if ($_SESSION['Items']->trans_type==ST_SALESQUOTE){
	 $is_cash_cust= is_cash_customer($_POST['customer_id']);
	  if($is_cash_cust['cash_customer']=='1'){
		if (!get_post('cash_cust_name')) 
		{
			display_error(_("Pleasse enter the cash customer name."));
			set_focus('cash_cust_name');
			return false;
		} 
		if (!get_post('cash_cust_mobile')) 
		{
			display_error(_("Please enter the cash customer phone number."));
			set_focus('cash_cust_mobile');
			return false;
		} 
	  }
	}
	*/
	...
	if ($_SESSION['Items']->trans_type!=ST_SALESORDER && $_SESSION['Items']->trans_type!=ST_SALESQUOTE 
	 && $_SESSION['Items']->trans_type!=ST_SALESENQ   && !is_date_in_fiscalyear($_POST['OrderDate'])) {
		display_error(_("The entered date is out of fiscal year or is closed for further data entry."));
		set_focus('OrderDate');
		return false;
	}
	
	$cust_info = get_customer_to_order($_POST['customer_id']);
	
	/*
	if($cust_info['credit_limit_tag_id']==3){
		
	 $inv_dd = get_customer_invoice_last_due_date($_POST['customer_id']);
	 $inv_due_date = sql2date($inv_dd['due_date']);
	 $customer_record = get_customer_details(get_post('customer_id'), get_post('OrderDate'),'',$_POST['dimension_id']);
	
	
	   if (date1_greater_date2($_POST['OrderDate'], $inv_due_date) && $customer_record["Balance"]>0){
		display_error(_("The selected customer account reached above the credit limit + Due amount. Please contact the credit control personnel to discuss."));
		set_focus('customer_id');
		return false;
	  }
	
	  if(($_SESSION['Items']->get_trans_total() > $_SESSION['Items']->credit) && 
	  $_SESSION['Items']->trans_type!=ST_SALESQUOTE && 
	  $_SESSION['Items']->trans_type!=ST_SALESENQ)
	  {
		display_error(_("The selected customer account reached above the credit limit. Please contact the credit control personnel to discuss.."));
		set_focus('customer_id');
		return false;
	  }
	}
	
	if($cust_info['credit_limit_tag_id']==2){
	 if(($_SESSION['Items']->get_trans_total() > $_SESSION['Items']->credit) && 
	 $_SESSION['Items']->trans_type!=ST_SALESQUOTE && 
	 $_SESSION['Items']->trans_type!=ST_SALESENQ)
	 {
	...
	*/
	...
	if ($_SESSION['Items']->payment_terms['cash_sale'] == 0) {
		...
		if ($_SESSION['Items']->trans_type != ST_SALESQUOTE && $_SESSION['Items']->trans_type != ST_SALESENQ && strlen($_POST['delivery_address']) <= 1) {
		...
		if ($_POST['ov_roundoff'] == "")
			$_POST['ov_roundoff'] = price_format(0);
		//Final Discount
		if($_SESSION['Items']->trans_type == ST_SALESQUOTE || $_SESSION['Items']->trans_type == ST_SALESORDER || $_SESSION['Items']->trans_type == ST_SALESINVOICE){
	...
	if($_SESSION['Items']->trans_type==ST_SALESINVOICE || $_SESSION['Items']->trans_type==ST_CUSTDELIVERY)
	{
		$credit_limit_tag_id = get_credit_limit_tag_id($_POST['customer_id']);
		
		
		 $credit_limit= get_customer_wise_credit_limit($_POST['customer_id']);
	     $current_balce= get_customer_wise_current_balance_without_pdc($_POST['customer_id']);
	     $dos= get_customer_wise_pending_do($_POST['customer_id']);
	     $pdc= get_customer_wise_pdc_in_hand($_POST['customer_id']);
	
	     $bill_amt = $credit_limit- $current_balce - $dos + $pdc;
		
		
	  //if($cust_info['cash_customer']==0 && (input_num('amount_total')>$_SESSION['Items']->credit) && $credit_limit_tag_id!=1)
		  
	    if($cust_info['cash_customer']==0 && (input_num('amount_total')>$bill_amt) && $credit_limit_tag_id!=1)
	  {
	    if($cust_info['customer_lock']==0 && $cust_info['customer_unlock_request']==0){
		 $ret_error = "Please contact your admin to unlock the customer.  Request to <a target='_blank' href='$path_to_root/inventory/manage/customer_unlock_request.php?debtor_no=" . $_POST['customer_id'] . "'>Unlock</a>";
		 display_error($ret_error);
		 set_focus('customer_id');
		 return false;
	    }
        if($cust_info['customer_lock']==1 && $cust_info['customer_unlock_request']==1)
		{
...
if (isset($_POST['ProcessOrder']) && can_process()) {
	...
	if(isset($_POST['ModifyInvNumber'])){
     $modify_inv_number = $_POST['ModifyInvNumber'];
     void_sales_invoice(ST_SALESINVOICE, $modify_inv_number);
	 $_SESSION['Items']->inv_edit_ref=$_POST['ref'];
	 
   } 
   
  
   if(isset($_POST['ModifyDelNumber'])){
     $modify_del_number = $_POST['ModifyDelNumber'];
	 void_sales_delivery(ST_CUSTDELIVERY, $modify_del_number);
	 $_SESSION['Items']->inv_edit_ref=$_POST['ref'];
   } 
   
   $total_credit_used = 0;
   $invoice = $_SESSION['Items'];
   foreach ($invoice->line_items as $taxitem) {
	   //display_error(http_build_query($taxitem));
	   $total_credit_used +=  $taxitem->quantity * $taxitem->price;
   }
   $total_credit_used +=$items_added_tax;
   
   
   $total_credit_used = $invoice->credit - $total_credit_used;
   
   
   $customer_lock = get_cust_release($invoice->customer_id);
   
  /* if($customer_lock == 0 && $total_credit_used<0 && $invoice->trans_type == ST_SALESINVOICE)
   {
	   display_error("The customer has exceeded the credit limit.");
   }
   else
   { */
		...
		if ($ret == -1)
		{
			...
			$ref = $Refs->get_next($_SESSION['Items']->trans_type, null, array('date' => Today(),'inv_type'=>get_post('invoice_type')));
		...
		else
		{
			...
			processing_end();
			if($invoice->trans_type == ST_SALESINVOICE)
			{
				update_customer_lock($invoice->customer_id);
			}
			
			if ($modified) {
				if ($trans_type == ST_SALESQUOTE)
					meta_forward($_SERVER['PHP_SELF'], "UpdatedQU=$trans_no");
				else if($trans_type == ST_SALESENQ)
					meta_forward($_SERVER['PHP_SELF'], "UpdatedEnq=$trans_no");
				else	
...
function check_item_data()
{
    ...
    if($_SESSION['Items']->trans_type!=ST_SALESENQ){
	   ...
	   if (input_num('qty') == 0)
	   {
		...
		elseif (!check_num('qty', 0) || !check_num('discount_percent', 0, 100)) {
    ...
    if($_SESSION['Items']->trans_type == ST_CUSTDELIVERY || $_SESSION['Items']->trans_type == ST_SALESINVOICE || $_SESSION['Items']->trans_type == ST_SALESORDER){
		if (!check_num('foc_quantity', 0)) {
			display_error( _("The item could not be updated because you are attempting to set the foc quantity ordered to less than 0"));
			set_focus('foc_quantity');
			return false;
		}
	}
	...
	if($_SESSION['Items']->trans_type==ST_SALESINVOICE || $_SESSION['Items']->trans_type==ST_CUSTDELIVERY)
	{
        ...
        foreach ($invoice->line_items as $lineitem) {
	         $total_line_item +=  round(($lineitem->quantity * $lineitem->price)-($lineitem->quantity * $lineitem->price * ($lineitem->discount_percent/100)), user_price_dec());
        }
		
		//display_error($total_line_item);
		
	    if($cust_info['cash_customer']==0 && ($total_line_item>$_SESSION['Items']->credit))
	    {
	      display_error( _("The item could not be updated because you are attempting to set the item total greater than customer credit limit"));
		  set_focus('price');
		  return false;
	    }
	}
...
function handle_update_item()
{
	...
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Items']->update_cart_item($_POST['LineNo'],
		 ...
		 input_num('discount_percent'), $_POST['item_description'],
		 input_num('disc_amount'), $_POST['units'],input_num('foc_quantity'));
	}
	unset($_POST['_stock_id_edit'], $_POST['stock_id'],$_POST['discount_percent'] ,
	$_POST['disc_amount'], $_POST['units'],$_POST['foc_quantity']);
	page_modified();
  line_start_focus();
}
...
function handle_new_item()
{
	...
	add_to_order($_SESSION['Items'], get_post('stock_id'), input_num('qty'),
		input_num('price'), input_num('discount_percent'), get_post('stock_id_text'),
		input_num('disc_amount'),get_post('units'),input_num('foc_quantity'));

	unset($_POST['_stock_id_edit'], $_POST['stock_id'],$_POST['disc_amount'], 
	$_POST['units'],$_POST['foc_quantity']);
	page_modified();
	line_start_focus();
}
...
function  handle_cancel_order()
{
	...
	} elseif ($_SESSION['Items']->trans_type == ST_SALESENQ)
	{
		if ($_SESSION['Items']->trans_no != 0) 
			delete_sales_order(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);
		display_notification(_("This sales Enquirie has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Enquirie"), "/sales/sales_order_entry.php?NewEnq=Yes");
	}
	else { // sales order
		if ($_SESSION['Items']->trans_no != 0) {
			...
			{
				copy_to_cart();
				meta_forward($path_to_root . '/sales/inquiry/sales_order_cancel_entry.php?order_no='.$order_no.'&cancel_status=1&del_status=1&');    
                /*
                ...
                */
				...
				copy_to_cart();
				meta_forward($path_to_root . '/sales/inquiry/sales_order_cancel_entry.php?order_no='.$order_no.'&cancel_status=1');
...
{ 
	...
	/* else if(isset($_GET['NewEnqToSalesQuote'])){
		
		$trans_no = $_GET['NewEnqToSalesQuote'];
		$doc = new Cart(ST_SALESENQ, $trans_no, true);
		$doc->Comments = _("Sales Enquiry") . " # " . $trans_no;
		$_SESSION['Items'] = $doc;
		...
		$trans_no = $_GET['NewEnqToSalesQuote'];
		$doc = new Cart(ST_INSSALESENQ, $trans_no, true);
		$doc->Comments = _("Sales Enquiry") . " # " . $trans_no;
		$_SESSION['Items'] = $doc;
		
	}
	 else if(isset($_GET['ModifyInvNumber'])){
			$trans_no = $_GET['ModifyInvNumber'];
			$_SESSION['Items'] = new Cart(ST_SALESINVOICE, array($trans_no));				
			$_SESSION['Items']->get_sales_invoice_items($trans_no,ST_SALESINVOICE);
			$_SESSION['Items']->trans_no=0;
			$_SESSION['Items']->order_no=null;	
			$_SESSION['Items']->src_docs=[];	
			

			
			foreach($doc->line_items as $line_no => $line) {
				$_SESSION['Items']->line_items[$line_no]->src_id = 0;
				$_SESSION['Items']->line_items[$line_no]->src_no = 0;
			} 		
	} 
	
	else if(isset($_GET['ModifyDelNumber'])){
			$trans_no = $_GET['ModifyDelNumber'];
			$_SESSION['Items'] = new Cart(ST_CUSTDELIVERY, array($trans_no));				
			$_SESSION['Items']->get_sales_delivery_items($trans_no,ST_CUSTDELIVERY);
			$_SESSION['Items']->trans_no=0;
			$_SESSION['Items']->order_no=null;	
			$_SESSION['Items']->src_docs=[];	
	...
	/* else if(isset($_GET['ModifyInvNumber'])){
		...
		$trans_no = $_GET['ModifyInvNumber'];
		$doc = new Cart(ST_SALESINVOICE, $trans_no);
		 $doc->trans_type = ST_SALESINVOICE;	
		$_SESSION['Items'] = $doc;
		$_SESSION['Items']->trans_no=0;
		$_SESSION['Items']->order_no=null;
		$_SESSION['Items']->src_id=0;
		$_SESSION['Items']->src_no=0;
		
		foreach($doc->line_items as $line_no => $line) {
			$doc->line_items[$line_no]->src_id = 0;
			$doc->line_items[$line_no]->src_no = 0;
		} 
	...
	elseif($type != ST_SALESORDER && $type != ST_SALESQUOTE && $type != ST_SALESENQ && $trans_no != 0) { // this is template
...
if ($_SESSION['Items']->trans_type == ST_SALESINVOICE) {
	...
	$idate = _("Enquiry Date:");
	$orderitems = _("Sales Enquiry Items");
	$deliverydetails = _("Enter Delivery Details and Confirm Enquiry");
	$cancelorder = _("Cancel Enquiry");
	$porder = _("Place Enquiry");
	$corder = _("Commit Enquiry Changes");
} else {
	$idate = _("Order Date:");
...
hidden('cart_id');
 ...
 hidden('ModifyDelNumber', $_GET['ModifyDelNumber']);
