...
set_page_security( @$_SESSION['Items']->trans_type,
	array(	ST_SALESORDER=>'SA_SALESORDER',
			ST_INSSALESQUOTE => 'SA_INSSALESQUOTE',
			ST_SALESENQ => 'SA_SALESENQ',
			ST_INSSALESENQ => 'SA_INSSALESENQ',
	...
	array(	'NewOrder' => 'SA_SALESORDER',
			...
			'NewQuotation' => 'SA_INSSALESQUOTE',
			'ModifyQuotationNumber' => 'SA_INSSALESQUOTE',
			'NewQuoteToSalesOrder' => 'SA_INSSALESQUOTE',
			'AddedQU' => 'SA_INSSALESQUOTE',
			'UpdatedQU' => 'SA_INSSALESQUOTE',
			'NewEnquiry' => 'SA_INSSALESENQ',
			'ModifyEnquiryNumber' => 'SA_INSSALESENQ',
			'AddedEnq' => 'SA_INSSALESENQ',
			'UpdatedEnq' => 'SA_INSSALESENQ',
			'NewEnqToQuote' => 'SA_INSSALESENQ',
...
if (isset($_GET['NewDelivery']) && is_numeric($_GET['NewDelivery'])) {
	...
	create_cart(ST_INSSALESQUOTE, $_GET['ModifyQuotationNumber']);

} 
elseif (isset($_GET['ModifyEnquiryNumber']) && is_numeric($_GET['ModifyEnquiryNumber'])) {

	$help_context = 'Modifying Sales Enquiry';
	$_SESSION['page_title'] = sprintf( _("Modifying Sales Enquiry # %d"), $_GET['ModifyEnquiryNumber']);
	create_cart(ST_INSSALESENQ, $_GET['ModifyEnquiryNumber']);
...
elseif (isset($_GET['ModifyQuotationNumber']))
	check_is_editable(ST_INSSALESQUOTE, $_GET['ModifyQuotationNumber']);
elseif 	(isset($_GET['ModifyEnquiryNumber']))
	check_is_editable(ST_INSSALESENQ, $_GET['ModifyEnquiryNumber']);
...
/*
if((isset($_POST['sap_code'])) &&  (!isset($_POST['AddItem'])))
{
	if($_POST['sap_code']!='')
	{
		$item_description =get_item_description_by_sap_code($_POST['sap_code']);
		if($item_description)
		{
		 $_POST['item_description']=$item_description;
		  $Ajax->activate('item_description');
		}
	}
}

if((isset($_POST['item_number'])) &&  (!isset($_POST['AddItem'])))
{
	if($_POST['item_number']!='')
	{
		$item_description =get_item_description_by_item_code($_POST['item_number']);
		if($item_description)
		{
		 $_POST['item_description']=$item_description;
		  $Ajax->activate('item_description');
...
if((isset($_POST['mesc'])) &&  (!isset($_POST['AddItem'])))
{
	if($_POST['mesc']!='')
	{
		$item_description =get_item_description_by_mesc_no($_POST['mesc']);
		if($item_description)
		{
		 $_POST['item_description']=$item_description;
		  $Ajax->activate('item_description');
...
if((isset($_POST['tag_no'])) &&  (!isset($_POST['AddItem'])))
{
	if($_POST['tag_no']!='')
	{
		$item_description =get_item_description_by_tag_no($_POST['tag_no']);
		if($item_description)
		{
		 $_POST['item_description']=$item_description;
		  $Ajax->activate('item_description');
...
if (isset($_GET['AddedID'])) {
	...
	submenu_view(_("&View This Quotation"), ST_INSSALESQUOTE, $order_no);

	submenu_print(_("&Print This Quotation"), ST_INSSALESQUOTE, $order_no, 'prtopt');
	submenu_print(_("&Email This Quotation"), ST_INSSALESQUOTE, $order_no, null, 1);
	set_focus('prtopt');
	...
	submenu_option(_("Add an Attachment"), "/admin/attachments.php?filterType=".ST_INSSALESQUOTE."&trans_no=$order_no");
...
elseif (isset($_GET['AddedEnq'])) {
	...
	submenu_view(_("&View This Enquiry"), ST_INSSALESENQ, $order_no);

	submenu_print(_("&Print This Enquiry"), ST_INSSALESENQ, $order_no, 'prtopt');
	submenu_print(_("&Email This Enquiry"), ST_INSSALESENQ, $order_no, null, 1);
	set_focus('prtopt');
	
	submenu_option(_("Make &Sales Quotation Against This Enquiry"),
		"/sales/sales_order_entry.php?NewEnqToSalesQuote=$order_no");

	submenu_option(_("Enter a New &Enquiry"),	"/sales/sales_order_entry2.php?NewEnquiry=0");
	
	submenu_option(_("Add an Attachment"), "/admin/attachments.php?filterType=".ST_INSSALESENQ."&trans_no=$order_no");
...
elseif (isset($_GET['UpdatedQU'])) {
	...
	submenu_option(_("Select A Different &Quotation"),
		"/sales/inquiry/sales_orders_view.php?type=".ST_INSSALESQUOTE);
...
elseif (isset($_GET['UpdatedEnq'])) {
	...
	submenu_option(_("Make &Sales Quotation Against This Enquiry"),
		"/sales/sales_order_entry2.php?NewEnqToQuote=$order_no");

	submenu_option(_("Select A Different &Enquiry"),
		"/sales/inquiry/sales_insquote_view.php?type=".ST_INSSALESENQ);
...
function copy_to_cart()
{
	...
	$cart->deviations = $_POST['deviations'];
	$cart->important_notes = $_POST['important_notes'];
	$cart->customer_order_type = $_POST['customer_order_type'];
	$cart->revisions  = $_POST['revisions'];
...
function copy_from_cart()
{
	...
	$_POST['deviations'] = $cart->deviations;
	$_POST['customer_order_type'] = $cart->customer_order_type;
	$_POST['important_notes'] = $cart->important_notes;
	$_POST['revisions'] = $cart->revisions;
...
function can_process() {
	...
	if ($_SESSION['Items']->trans_type!=ST_SALESORDER && $_SESSION['Items']->trans_type!=ST_INSSALESQUOTE && $_SESSION['Items']->trans_type!=ST_INSSALESENQ && !is_date_in_fiscalyear($_POST['OrderDate'])) {
	...
	if ($_SESSION['Items']->payment_terms['cash_sale'] == 0) {
		...
		if ($_SESSION['Items']->trans_type != ST_INSSALESQUOTE && ($_SESSION['Items']->trans_type != ST_INSSALESENQ) && strlen($_POST['delivery_address']) <= 1) {
		...
		if (!is_date($_POST['delivery_date'])) {
			if ($_SESSION['Items']->trans_type==ST_INSSALESQUOTE && $_SESSION['Items']->trans_type==ST_INSSALESENQ)
...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Items']->update_temp2_cart_item($_POST['LineNo'],$_POST['item_number'],$_POST['item_description'],
		 input_num('qty'), $_POST['units'],input_num('unit_price'), input_num('qty')*input_num('unit_price'));
	}
	
	
	unset($_POST['item_number']);
	unset($_POST['item_description']);
	unset($_POST['qty']);
	unset($_POST['units']);
...
function handle_new_item()
{
	...
	add_to_temp2_order($_SESSION['Items'],get_post('item_number'),get_post('item_description'), input_num('qty'),get_post('units'),input_num('unit_price'), input_num('qty')*input_num('unit_price'));
...
function  handle_cancel_order()
{
	...
	} elseif ($_SESSION['Items']->trans_type == ST_INSSALESQUOTE)
	{
	 ...
	 elseif ($_SESSION['Items']->trans_type == ST_INSSALESENQ){
		 
		 if ($_SESSION['Items']->trans_no != 0) 
			delete_sales_order(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);
		display_notification(_("This sales enquiry has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Enquiry"), "/sales/sales_order_entry.php?NewEnquiry=Yes");
...
{ 
	...
	if (isset($_GET['NewQuoteToSalesOrder']))
	{
		...
		$doc = new Cart(ST_INSSALESQUOTE, $trans_no, true);
		// $doc->Comments = _("Sales Quotation") . " # " . $trans_no;
		$_SESSION['Items'] = $doc;
	}	
	else if(isset($_GET['NewEnqToQuote']))
	{
		$trans_no = $_GET['NewEnqToQuote'];
		$doc = new Cart(ST_INSSALESENQ, $trans_no, true);
		// $doc->Comments = _("Sales Enquiry") . " # " . $trans_no;
		$_SESSION['Items'] = $doc;
	}
	elseif($type != ST_SALESORDER && $type != ST_INSSALESQUOTE && $type != ST_INSSALESENQ && $trans_no != 0) { // this is template
...
if ($customer_error == "") {
	...
	echo "<tr><td>";
	display_order_temp2_summary($orderitems, $_SESSION['Items'], true);
	echo "</td></tr>";
