...
elseif (isset($_GET['type']) && $_GET['type'] == ST_SALESENQ)
	$trans_type = ST_SALESENQ;		
else
...
if ($trans_type == ST_SALESORDER)
{
	...
	$_POST['order_view_mode'] = "Enquiries";
	$_SESSION['page_title'] = _($help_context = "Search All Sales Enquires");
}
else
{
...
{
	global $trans_type;
	if ($trans_type == ST_SALESQUOTE || $trans_type == ST_SALESENQ)
...
function edit_link($row) 
{
	...
	{
		// only admin can edit after delivery  # Ramesh
		$user_id = $_SESSION['wa_current_user']->username;
		$role_id =check_user_role_id($user_id);
		if ($row['TotDelivered'] > 0 && $role_id!=2 )
		   return ''; 
       else	
...
/*	if ($row['ord_payments'] + $row['inv_payments'] < $row['prep_amount'])
	...
	$advance_amount=round(get_sales_advances_amount($row['order_no']),3);
	$sales_order_amount=round(get_sales_order_amount($row['order_no']),3);
	


if($row['payment_terms']==1)
	{
	if($sales_order_amount<=$advance_amount)
	{
    return pager_link( _("Dispatch"),
				"/sales/customer_delivery.php?OrderNumber=" .$row['order_no'], ICON_DOC);
	}
...
function documinvoice_link($row)
{
	global $trans_type;
	if ($trans_type == ST_SALESORDER)
  		return pager_link( _("Invoice"),
			"/sales/customer_doinvoice.php?OrderNumber=" .$row['order_no'], ICON_DOC);
	else
		return '';
}
...
{
  return pager_link( _("Sales Quotation"),
	"/sales/sales_order_entry.php?NewEnqToSalesQuote=" .$row['order_no'], ICON_DOC);
}


function fun_delivery_time($row)
{
  global $delivery_times;	
  return $delivery_times[$row['delivery_time']];
}
...
ref_cells(_("Quote Ref"), 'OrderReference', '',null, '', true);
}else if ($trans_type == ST_SALESENQ){
    ref_cells(_("RFQ Ref"), 'OrderReference', '',null, '', true);
}else
...
stock_items_list_cells_sales(_("Item:"), 'SelectStockFromList', null, true, true);
...
$sql = get_sql_for_sales_orders_view($trans_type, get_post('OrderNumber'), get_post('order_view_mode'),
	...
	get_post('customer_id'),$_POST['Dimension'], $_POST['Dimension2']);
...
if ($trans_type == ST_SALESORDER)
	...
	$cols = array(
		_("Enq #") => array('fun'=>'view_link', 'align'=>'right', 'ord' =>''),
		_("RFQ Ref") => array('type' => 'sorder.reference', 'ord' => '') ,
		_("Customer") => array('type' => 'debtor.name' , 'ord' => '') ,
		_("Branch"), 
		_("Cust Order Ref"),
		_("Enquiry Date") => array('type' =>  'date', 'ord' => ''),
		_("Required By") =>array('type'=>'date', 'ord'=>''),
		_("Delivery To"), 
		//_("Enq Total") => array('type'=>'amount', 'ord'=>''),
		'Type' => 'skip',
		_("Currency") => array('align'=>'center')
	);	
else
...
if ($_POST['order_view_mode'] == 'OutstandingOnly') {
	array_append($cols, array(
	    _("Delivery Time") => array('insert'=>true, 'fun'=>'fun_delivery_time','align'=>'center'),
		array('insert'=>true, 'fun'=>'edit_link'),
		array('insert'=>true, 'fun'=>'dispatch_link'),
		array('insert'=>true, 'fun'=>'prt_link'),
		array('insert'=>true, 'fun'=>'documinvoice_link')));
...
elseif ($trans_type == ST_SALESENQ) {
	 array_append($cols,array(
					array('insert'=>true, 'fun'=>'edit_link'),
					array('insert'=>true, 'fun'=>'quote_link'),
					array('insert'=>true, 'fun'=>'prt_link')));
} 
