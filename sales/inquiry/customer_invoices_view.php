...
page(_($help_context = "Invoice - Signed Copy Collection"), isset($_GET['customer_id']), false, "", $js);
...
function edit_link($row)
{
	...
	if ($row['type'] == ST_SALESINVOICE){
	 if (user_check_access('SA_SALES_INV_EDIT')) {
        return $row['type'] == ST_CUSTCREDIT && $row['order_'] ? '' : trans_editor_link($row['type'], $row['trans_no']);
	 }		 
	 else{	
...
function upload_link($row) 
{
	if($row['signed_collection_status'] == 0){
  return user_check_access('SA_SALES_INVOICE_SIGNED') ? pager_link( _("Upload"), "/sales/inquiry/upload_signed_copy_invoice.php?TransNo=" . $row["trans_no"],ICON_DOC) : '';
	}else 
	{
		return "Collected";
	}
...
if (!$page_nested)
 ...
 // cust_allocations_list_cells(null, 'filterType', 1, true, true);

 //ref_cells(_("Invoice Reference No.:"), 'trans_no');   
ref_cells(_("Reference No.:"), 'ref_no', '',null, '', true);
...
invoice_collection_list_cells(_("Invoice Collection :"), 'invoice_collect', null, true,true);
...
{
	$customer_record = get_customer_details(get_post('customer_id'), get_post('TransToDate'),'',$_POST['Dimension']);
...
$sql = get_sql_for_customer_invoice_inquiry(get_post('TransAfterDate'), get_post('TransToDate'),
	get_post('customer_id'), 1, check_value('show_voided'),$_POST['Dimension'], $_POST['Dimension2'], $_POST['ref_no'], $_POST['invoice_collect']);
...
$cols = array(
	...
	_("Balance") => array('align'=>'right', 'type'=>'amount'),
		...
		array('insert'=>true, 'fun'=>'upload_link'),
