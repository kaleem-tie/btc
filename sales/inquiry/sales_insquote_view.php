...
elseif (isset($_GET['type']) && $_GET['type'] == ST_INSSALESQUOTE)
	$trans_type = ST_INSSALESQUOTE;
elseif (isset($_GET['type']) && $_GET['type'] == ST_INSSALESENQ)
	$trans_type = ST_INSSALESENQ;	
else
...
elseif(($_GET['type'] == ST_INSSALESENQ)){
	
	$_POST['order_view_mode'] = "Enquiries";
	$_SESSION['page_title'] = _($help_context = "Search All Sales Enquiries");
}
else
{
...
{
	global $trans_type;
	if ($trans_type == ST_INSSALESQUOTE || $trans_type == ST_INSSALESENQ)
...
function type_fun($row)
{
    global $customer_order_type;
    
    return $customer_order_type[$row['customer_order_type']];
    
}

function user_fun($row)
{
    return $row['transaction_user'];
}
...
function quote_link($row){
	return pager_link( _("Sales Quote"),
...
function tmpl_checkbox($row)
{
	...
	if ($trans_type == ST_INSSALESQUOTE || $trans_type == ST_INSSALESENQ || !check_sales_order_type($row['order_no']))
		return '';
...
$sql = get_sql_for_sales_insquote_view($trans_type, get_post('OrderNumber'), get_post('order_view_mode'),
	...
	get_post('customer_id'),get_post('customer_order_type'),
	$_POST['Dimension'], $_POST['Dimension2']);

if ($trans_type == ST_SALESORDER)
	$cols = array(
		_("Order #") => array('fun'=>'view_link', 'align'=>'center'),
		//_("Sales Type") => array('fun'=>'type_fun', 'align'=>'left'),
	...
	$cols = array(
		_("Quote #") => array('fun'=>'view_link', 'align'=>'center'),
		// _("Sales Type") => array('fun'=>'type_fun', 'align'=>'left'),
		_("Ref"),
		_("Customer"),
		_("Branch"), 
	...
	$cols = array(
		_("ENQ #") => array('fun'=>'view_link', 'align'=>'center'),
	// 	_("Sales Type") => array('fun'=>'type_fun', 'align'=>'left'),
		_("Ref"),
		_("Customer"),
		_("Branch"), 
