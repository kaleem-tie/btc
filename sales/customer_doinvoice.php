...
if (isset($_GET['ModifyDelivery'])) {
	...
	$_SESSION['page_title'] = _($help_context = "Invoice Items for a Sales Order");
	processing_start();
}
...
if (isset($_GET['AddedID'])) {
	...
	display_notification_centered(sprintf(_("Invoice # %d has been entered."),$dispatch_no));

	display_note(get_customer_trans_view_str(ST_SALESINVOICE, $dispatch_no, _("&View This Invoice")), 0, 1);

	display_note(print_document_link($dispatch_no, _("&Print Invoice Note"), true, ST_SALESINVOICE));
	display_note(print_document_link($dispatch_no, _("&Email Invoice Note"), true, ST_SALESINVOICE, false, "printlink", "", 1), 1, 1);
	display_note(print_document_link($dispatch_no, _("P&rint as Packing Slip"), true, ST_SALESINVOICE, false, "printlink", "", 0, 1));
	display_note(print_document_link($dispatch_no, _("E&mail as Packing Slip"), true, ST_SALESINVOICE, false, "printlink", "", 1, 1), 1);

	display_note(get_gl_view_str(13, $dispatch_no, _("View the GL Journal Entries for this Dispatch")),1);
	
	hyperlink_params("$path_to_root/admin/attachments.php", _("Add an Attachment"), "filterType=".ST_SALESINVOICE."&trans_no=$dispatch_no");	
	...
	hyperlink_params("$path_to_root/admin/attachments.php", _("Add an Attachment"), "filterType=".ST_CUSTDELIVERY."&trans_no=$delivery_no");
...
function check_quantities()
{
	...
	foreach ($_SESSION['Items']->line_items as $line=>$itm) {
		...
		$_SESSION['Items']->line_items[$line]->foc_quantity = input_num('FOC'.$line);
...
if (isset($_POST['process_delivery']) && check_data()) {
	...
	$delivery_no = $dn->write($bo_policy,1);
...
if ($dim > 0) {
	...
	dimensions_list_cells(null, 'dimension_id', null, false, ' ', false, 1, false);
	end_row();
}		
else
...
if ($dim > 1) {
	...
	dimensions_list_cells(null, 'dimension2_id', null, false, ' ', false, 2, false);
	end_row();
}		
else
...
display_heading(_("DO + Invoice Items"));
div_start('Items');
start_table(TABLESTYLE, "width='80%'");

$new = $_SESSION['Items']->trans_no==0;
$th = array(_("S.No."), _("Item Code"), _("Item Description"), 
	$new ? _("Ordered") : _("Max. delivery"), _("FOC Quantity"), _("Units"), $new ? _("Delivered") : _("Invoiced"),
	_("This Delivery"), _("Price"), _("Tax Type"), _("Discount"), _("Discount Amt."), _("Total"));
...
foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
	...
	if (has_stock_holding($ln_itm->mb_flag) && $ln_itm->qty_dispatched) {
		...
		//display_error($ln_itm->units);
		
		if($ln_itm->units==1){
			$check_dn_qty = ($ln_itm->qty_done-$ln_itm->qty_dispatched)*1;
		}
        else if($ln_itm->units==2){	
		    $sec_unit_info = get_item_sec_unit_info($ln_itm->stock_id);
           $check_dn_qty = ($ln_itm->qty_done-$ln_itm->qty_dispatched)/
		   $sec_unit_info['conversion_factor'];
        }	


		if ($check = check_negative_stock($ln_itm->stock_id, $check_dn_qty, $_POST['Location'], $_POST['DispatchDate']))
	...
	$display_discount_amt = percent_format(($ln_itm->qty_dispatched * $ln_itm->price) *$ln_itm->discount_percent/100);
	...
	label_cell($display_discount_amt, "nowrap align=right");
...
submit_center_last('process_delivery', _("Process Invoice"),
