	...
	foreach ($_SESSION['Items']->line_items as $line_no=>$itm) {
		...
		if (isset($_POST['Line'.$line_no])) {
			if (check_num('Line'.$line_no, 0, $itm->quantity)) {
				...
				$_SESSION['Items']->line_items[$line_no]->foc_qty_sent =
				  input_num('Line_foc'.$line_no);	
			}
			else {
				$ok = 0;
			}
...
end_row();
    ...
    $th = array(_("S.No."), _("Item Code"), _("Item Description"), _("Invoiced Quantity"),_("FOC Quantity"), _("Units"),
    	_("Credit Quantity"),_("Credit FOC Quantity"), _("Price"), _("Discount %"), _("Total"));
    table_header($th);
	...
	if($ln_itm->units==1){
	  ...
	  label_cell($item_info["units"], "align=right");
		...
		amount_cells(null, 'Line_foc'.$line_no,  number_format2($ln_itm->foc_qty_sent, $dec),
			null, null, $dec);	
			
    	$line_total =($ln_itm->qty_dispatched * $ln_itm->price * (100 - $ln_itm->discount_percent)*0.01);

    	amount_cell($ln_itm->price);
    	percent_cell($ln_itm->discount_percent);
	...
	small_amount_cells(null, "ov_roundoff", price_format(get_post('ov_roundoff')));
    end_row();
