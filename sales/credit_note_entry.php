...
function check_item_data()
{
	...
	if (!check_num('discount_percent', 0, 100)) {
...
{
	if($_SESSION['Items']->line_items[$_POST['line_no']]->discount_percent != input_num('discount_percent') ){
			$_POST['disc_amount'] = (input_num('price')*input_num('qty'))*(input_num('discount_percent')/100);
	}
	if($_SESSION['Items']->line_items[$_POST['line_no']]->disc_amount != input_num('disc_amount') ){
	...
	if ($_POST['UpdateItem'] != "" && check_item_data()) {
		$_SESSION['Items']->update_cart_item($_POST['line_no'], input_num('qty'),
			input_num('price'), input_num('discount_percent'), $_POST['item_description'],
			input_num('disc_amount'), $_POST['units']);
	}
    line_start_focus();
}
