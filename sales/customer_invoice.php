...
if(isset($_REQUEST['DeliveryNumber']))
{
	 $_POST['DeliveryNumber'] = $_REQUEST['DeliveryNumber'];
}
...
if (isset($_GET['AddedID'])) {
	...
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Print This Invoice"), true, ST_SALESINVOICE_REP));
	
	
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Email This Invoice"), true, ST_SALESINVOICE_REP, false, "printlink", "", 1),1);
...
if ( (isset($_GET['DeliveryNumber']) && ($_GET['DeliveryNumber'] > 0) )
   ...
   $inv_info = get_customer_trans($_GET['ModifyInvoice'],ST_SALESINVOICE);
	
   if($inv_info['signed_collection_status']==1 && !user_check_access('SA_SALES_INV_EDIT'))
   check_sales_invoice_editable(_("You have no edit access to sales invoice transactions."));
...
elseif (!processing_active()) {
	...
	display_error(_("Selected quantity cannot be less than quantity credited not more than quantity not invoiced yet."));
}  
...
$order_details = get_order_details($_SESSION['Items']->trans_no);


		if($order_details['reference']=='auto')
		{
				//ravi
				$_SESSION['Items']->line_items[$line_no]->price =
							input_num('LinePrice'.$line_no);
							
				$_SESSION['Items']->line_items[$line_no]->discount_percent =
							input_num('LineDiscount'.$line_no);			
		}					
		...
		if (isset($_POST['Line'.$line_no.'Desc'])) {
		  ...
		  if($ok == 0 && $order_details['reference']!='auto')
			{
		$_SESSION['Items']->line_items[$line_no]->qty_dispatched =
				    input_num('Line'.$line_no);
			}
			
			$_SESSION['Items']->line_items[$line_no]->foc_quantity = input_num('FOC'.$line_no);
	  }
	  
		if($order_details['reference']!='auto')  // no need to check the delivered qty
		return 1;
...
function check_data()
{
	...
	if($_SESSION["wa_current_user"]->invoice_entry_date_before_do==0)
	{
		$no_of_days_after = vaidateInvoiceDatewithDoDate($_POST['DeliveryNumber'],$_POST['InvoiceDate']);
		if($no_of_days_after<0)
		{
			display_error(_("The entered invoice date is invalid or before DO Date.")); 
			return false;
		}			
	}
...
if (isset($_POST['process_invoice']) && check_data()) {
	...
	{
	   $advances_info=get_sales_advances_info($_SESSION['Items']->customer_id);
   	   $advance_ids=array();
	   	   
	   while($sales_advance=db_fetch($advances_info))
	   {         
		          if(check_value('adv_'.$sales_advance['trans_no'])==1)
				  {
					  
				    array_push($advance_ids,$sales_advance['trans_no']);
				  }
		}
		$_SESSION['Items']->advance_ids=$advance_ids;
    }
	//end
    ...
    // ravi  auto delivery modification
	
/*	 $delivery_details = get_delivery_details($_SESSION['Items']->trans_no);
	 
	   display_error(json_encode($delivery_details));
	 
		 
	 if($delivery_details['reference']=='auto')
	 {
		  $auto_delivery_details = new Cart(ST_CUSTDELIVERY, array($delivery_details['trans_no']), true);
		  $_SESSION['DOItems'] = $auto_delivery_details;
		  
		   $_SESSION['Items']->trans_type=13;
		   $_SESSION['Items']->trans_no=$delivery_details['trans_no'];
		   
		   
		   for ($line_no = 0; $line_no < count($_SESSION['Items']->line_items); $line_no++) {
				$line = $_SESSION['Items']->line_items[$line_no];
				$line1 = $_SESSION['DOItems']->line_items[$line_no];
				
				 $line->id=$line1->id;
				 $line->src_id=$line1->src_id;
				 $line->src_docs=$line1->src_docs;
				
		 	}
			
			$delivery_no = $_SESSION['Items']->write(1);
	 } */
...
start_row();
label_cell(_("Invoice Type").":", "class='tableheader2'");
invoice_types_list_cells(null, 'invoice_type', null, true);
end_row();
...
if ($_SESSION['Items']->trans_no == 0) {
	...
	array('customer' => $_SESSION['Items']->customer_id,
			...
			'inv_type' => get_post('invoice_type')));
} else {
...
if ($prepaid)
{
 ...
 if (!isset($_POST['lpo_date']) || !is_date($_POST['lpo_date'])) {
	$_POST['lpo_date'] = $_SESSION['Items']->lpo_date;
}

start_row();
text_cells(_("LPO No:"), 'lpo_no', $_SESSION['Items']->lpo_no, 25, 25, _('LPO No'), "class='tableheader2'");
end_row();

start_row();
date_cells(_("LPO Date"), 'lpo_date', '', null, 0, 0, 0, "class='tableheader2'");
end_row();
...
end_row();
	...
	if($_SESSION['Items']->trans_type==ST_SALESINVOICE)
		...
		//div_start('advance');
		   ...
		   $sales_advances=get_sales_advances_info($_SESSION['Items']->customer_id);
			$flag=0;
			start_row();
...
if ($prepaid)
	$th = array(_("S.No."), _("Item Code"), _("Item Description"), _("Units"), _("Quantity"),
		_("Price"), _("Tax Type"), _("Discount"), _("Total"));
else
	$th = array(_("S.No."), _("Item Code"), _("Item Description"), _("Delivered"), _("FOC Quantity"), 
_("Units"), _("Invoiced"), _("This Invoice"), _("Price"), _("Tax Type"), _("Discount"), _("Discount Amt."),
_("Total"));
...
foreach ($_SESSION['Items']->line_items as $line=>$ln_itm) {
	...
	if (!$prepaid)
	{
	    //qty_cell($ln_itm->foc_quantity, false, $dec);
	  $ln_itm->foc_qty_sent = $ln_itm->foc_quantity-$ln_itm->foc_qty_sent;
	  small_qty_cells(null, 'FOC'.$line, qty_format($ln_itm->foc_qty_sent, $ln_itm->stock_id, $dec), null, null, $dec);
	}
	
	 if($ln_itm->units==1){
      $item_info = get_item_edit_info($ln_itm->stock_id);
	  label_cell($item_info["units"], "align=right");
     }
     else if($ln_itm->units==2){
		$sec_unit_info = get_item_sec_unit_info($ln_itm->stock_id);
	...
	$display_discount_percent = percent_format($ln_itm->discount_percent) . " %";

	$line_total = ($ln_itm->qty_dispatched * $ln_itm->price * (1 - $ln_itm->discount_percent/100));
	
	if($order_status=="Direct")
	{
		small_amount_cells(null,'LinePrice'.$line,$ln_itm->price);
		label_cell($ln_itm->tax_type_name);
	   //label_cell($display_discount_percent, "nowrap align=right");
	   small_amount_cells(null,'LineDiscount'.$line,$ln_itm->discount_percent);
	}
    else
	{
	...
	amount_cell($ln_itm->disc_amount);
...
small_amount_cells(null, 'ov_roundoff', null);
end_row();
$display_total = price_format(($inv_items_total + input_num('ChargeFreightCost') + $tax_total) + input_num('ov_roundoff'));
	...
	if($_SESSION['Items']->trans_type==ST_SALESINVOICE)
		{
			...
			{
			    if(check_value('adv_'.$sales_advance['trans_no'])==1)
				$total_advance+=$sales_advance['ov_amount'];
			}
			if($total_advance!=0)
			{
			  $display_balance_amt=price_format(($inv_items_total + input_num('freight_cost') + $tax_total)-$total_advance);
			  if(($inv_items_total + input_num('freight_cost') + $tax_total-$total_advance)<0)
			  $display_balance_amt=price_format(0);	  
			 	  
			  $display_advance=price_format(-$total_advance);
			 label_row(_("Total Advance (Incl. Tax)"), $display_advance, "colspan=$colspan align=right","align=right", 0);
			 label_row(_("Balance Amount"), $display_balance_amt, "colspan=$colspan align=right","align=right", 0);
			}
		}
...
hidden('DeliveryNumber',$_POST['DeliveryNumber']);
