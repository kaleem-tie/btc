...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_sales_area($selected_id, $_POST['description'],$_POST["sale_cust_region_id"],$_POST["sale_cust_wilayat_id"]);
			$note = _('Selected sales area has been updated');
    	} 
    	else 
    	{
    		add_sales_area($_POST['description'],$_POST["sale_cust_region_id"],$_POST["sale_cust_wilayat_id"]);
...
$th = array(_("Area Name"),_("Region"),_("Wilayat"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell(get_region_name($myrow["sale_cust_region_id"]));
	label_cell(get_wilayat_name($myrow["sale_cust_wilayat_id"]));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['sale_cust_region_id']=$myrow['sale_cust_region_id'];
		$_POST['sale_cust_wilayat_id']=$myrow['sale_cust_wilayat_id'];
	}
	hidden("selected_id", $selected_id);
} 

text_row_ex(_("Area Name:"), 'description', 30);
sales_cust_region_list_row( _("Customer Region:"), 'sale_cust_region_id', null);
sales_cust_wilayat_list_row( _("Customer Wilayat:"), 'sale_cust_wilayat_id', null); 
