...
{
	if(strlen($_POST['cust_code']) == 0) 
	{
		
		display_error( _('The Customer code cannot be empty'));
		set_focus('cust_code');
		return false;
	}
	if(strstr($_POST['cust_code'], " ") || strstr($_POST['cust_code'],"'") || 
		strstr($_POST['cust_code'], "+") || strstr($_POST['cust_code'], "\"") || 
		strstr($_POST['cust_code'], "&") || strstr($_POST['cust_code'], "\t")) 
	{
		
		display_error( _('The Customer code cannot contain any of the following characters -  & + OR a space OR quotes'));
		set_focus('cust_code');
		return false;
	...
	if(!$selected_id){
	 if(check_cust_code($_POST['cust_code']))
	 {
		  	
      		display_error( _("This Customer code is already assigned to another Customer."));
			set_focus('cust_code');
			return false;
	 }
	}
	else{
	 if(check_cust_code_edit($_POST['cust_code'],$selected_id))
	 {
	    display_error("Entered Customer Code is already existed!");
	    set_focus('cust_code');
	    return false;
	 }
	}
    ...
    /*
    if(!$selected_id)
	if(check_cust_phone($_POST['phone']))
	{
	    display_error("Entered Phone No is already existed!");
	    set_focus('phone');
	    return false;
	}
	
	if($selected_id)
	if(check_cust_phone_old($_POST['phone'],$selected_id))
	{
	    display_error("Entered Phone No is already existed!");
	    set_focus('phone');
	    return false;
	...
	*/
	...
	{
		display_error(_("The customer address cannot be empty."));
		set_focus('address');
		return false;
	} 
	...
	if(!$selected_id){
		...
		{
			display_error(_("The customer phone number must be numeric and is expected to be more than 8 characters."));
			set_focus('phone');
			return false;	
		}
		
	/* if(filter_var($_POST['email'], FILTER_VALIDATE_EMAIL) == false){
			display_error(_("The entered email is invalid!."));
			set_focus('email');
			return false;	
		}  */
	}
...
function handle_submit(&$selected_id)
{
	...
	if ($selected_id) 
	{
		update_customer($_POST['customer_id'], $_POST['CustName'], $_POST['cust_code']." - ".$_POST['CustName'], $_POST['address'],
			...
			input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'],check_value('cash_customer'),$_POST["cr_no"],$_POST["cr_date"],$_POST["sponser_name"],$_POST["nature_of_business"],$_POST["business_type_id"],$_POST["vat_reg"],$_POST["vat_renewal_date"],
			$_POST["vat_effective_date"],$_POST["tax_card_no"],$_POST["vat_status"],$_POST["phone"],$_POST["cust_code"],
			$_POST["legal_group_id"],$_POST["sale_cust_type_id"],$_POST["sale_cust_cat_id"],$_POST["country_id"],
			$_POST["sale_cust_class_id"],$_POST["sale_cust_group_comp_id"], $_POST["credit_limit_tag_id"],$_POST["website"]);
	...
	{ 	//it is a new customer
		...
		begin_transaction();
		add_customer($_POST['CustName'], $_POST['cust_code']." - ".$_POST['CustName'], $_POST['address'],
			$_POST['tax_id'], $_POST['curr_code'], $_POST['dimension_id'], $_POST['dimension2_id'],
			$_POST['credit_status'], $_POST['payment_terms'], input_num('discount') / 100, input_num('pymt_discount') / 100,
			input_num('credit_limit'), $_POST['sales_type'], $_POST['notes'], check_value('cash_customer'),
			$_POST["cr_no"],$_POST["cr_date"],$_POST["sponser_name"],$_POST["nature_of_business"],
			$_POST["business_type_id"],$_POST["vat_reg"],$_POST["vat_renewal_date"],$_POST["vat_effective_date"],$_POST["tax_card_no"],$_POST["vat_status"],$_POST["phone"],
			$_POST["cust_code"],$_POST["legal_group_id"],$_POST["sale_cust_type_id"],
			$_POST["sale_cust_cat_id"],$_POST["country_id"],$_POST["sale_cust_class_id"],$_POST["sale_cust_group_comp_id"], $_POST["credit_limit_tag_id"],$_POST["website"]);

		$selected_id = $_POST['customer_id'] = db_insert_id();
         
		if (isset($SysPrefs->auto_create_branch) && $SysPrefs->auto_create_branch == 1)
		{
        	add_branch($selected_id, $_POST['CustName'], $_POST['CustName'],
                ...
                $_POST['location'], $_POST['address'], 0, $_POST['ship_via'], $_POST['notes'], $_POST['bank_account'],$_POST['vat_reg'], $_POST['tax_id']);
                
        	$selected_branch = db_insert_id(); 
        
			add_crm_person($_POST['CustName'], $_POST['CustName'], '', $_POST['address'], 
...
function customer_settings($selected_id) 
{
	...
	else 
	{
		...
		$_POST['cust_code'] = $myrow["cust_code"];
		...
		$_POST['credit_limit_tag_id']  = $myrow["credit_limit_tag_id"];
		$_POST['notes']  = $myrow["notes"];
		$_POST['inactive'] = $myrow["inactive"];
		
		$_POST["cash_customer"]=$myrow["cash_customer"];
		$_POST["website"]=$myrow["website"];
		$_POST["cr_no"]=$myrow["cr_no"];
		$_POST["cr_date"]=sql2date($myrow["cr_date"]);
		$_POST['legal_group_id']=$myrow['legal_group_id'];
		$_POST['sale_cust_type_id']=$myrow['sale_cust_type_id'];
		$_POST['sale_cust_cat_id']=$myrow['sale_cust_cat_id'];
		$_POST['sale_cust_class_id']=$myrow['sale_cust_class_id'];
		$_POST['sale_cust_group_comp_id']=$myrow['sale_cust_group_comp_id'];	
		$_POST["sponser_name"]=$myrow["sponser_name"];
		$_POST["nature_of_business"]=$myrow["nature_of_business"];
		$_POST['business_type_id'] =$myrow['business_type_id'];
		...
		$_POST["tax_card_no"]=$myrow["tax_card_no"];
		$_POST["vat_status"]=$myrow["vat_status"];
		
		$_POST["created_user"]=$myrow["created_user"];
		$_POST["created_date_time"]=$myrow["created_date_time"];
		
		$_POST["modified_date_time"]=$myrow["modified_date_time"];
		$_POST["modified_user"]    =$myrow["modified_user"];
	}
	...
	text_row(_("Customer Code:<b style='color:red;'>*</b>"), 'cust_code', null, 21, 20, null);
	text_row(_("Customer Name:<b style='color:red;'>*</b>"), 'CustName', $_POST['CustName'], 40, 80);
	// text_row(_("Customer Short Name:<b style='color:red;'>*</b>"), 'cust_ref', null, 30, 30);
	
	check_row(_("Cash Customer:"), 'cash_customer');
	
	textarea_row(_("Address:<b style='color:red;'>*</b>"), 'address', $_POST['address'], 35, 5);
    text_row(_("Phone:<b style='color:red;'>*</b>"), 'phone', null, 32, 30);
	if(!$selected_id)
	email_row(_("E-mail:"), 'email', null, 35, 55);
    text_row(_("Website:"), 'website', null, 40, 80);
	text_row(_("CR No"), 'cr_no', null, 32, 30);
	date_row(_("CR Date"),'cr_date');
	sales_legal_group_list_row( _("Legal Group:"), 'legal_group_id', null);
	sales_cust_type_list_row( _("Customer Type:"), 'sale_cust_type_id', null);
	sales_cust_cat_list_row( _("Customer Category:"), 'sale_cust_cat_id', null);
	sales_cust_class_list_row( _("Customer Class:"), 'sale_cust_class_id', null);
	sales_cust_group_comp_list_row( _("Customer Group Company:"), 'sale_cust_group_comp_id', null);
	
	table_section(2);

	table_section_title(_("Buisness Details"));
	
	text_row(_("Name of the Owner/Sponsor:"), 'sponser_name', null, 32, 30);
	text_row(_("Nature of Business:"), 'nature_of_business', null, 32, 30);
	business_types_list_row(_("Business Type"),'business_type_id');
	
    vat_registered_list_row(_("VAT Registered"),'vat_reg',null,true);
	
	if($_POST['vat_reg']==1){
	text_row(_("VAT No:"), 'tax_id', null, 40, 40);
    ...
    text_row(_("Tax Card No :"), 'tax_card_no', null, 40, 40);
	}
	...
	credit_limit_tag_types_list_row(_("Credit Limit Tag:"),'credit_limit_tag_id'); 

	payment_terms_list_row(_("Payment Terms:"), 'payment_terms', $_POST['payment_terms']);
	credit_status_list_row(_("Credit Status:"), 'credit_status', $_POST['credit_status']); 
	$dim = get_company_pref('use_dimension');
	if ($dim >= 1)
		dimensions_list_row(_("Dimension")." 1:", 'dimension_id', $_POST['dimension_id'], false, " ", false, 1);
	if ($dim > 1)
		dimensions_list_row(_("Dimension")." 2:", 'dimension2_id', $_POST['dimension2_id'], false, " ", false, 2);
	if ($dim < 1)
	...
	if ($selected_id)  {
		...
		$created_user_data = get_user($myrow['created_user']);
		table_section_title(_("User Info"));
		label_row(_("Created Date :"),sql2date($myrow['created_date_time']));
		label_row(_("Created User :"),$created_user_data['user_id']);
		
		$modified_user_data = get_user($myrow['modified_user']);
		
		if($myrow['modified_user']!=''){
		label_row(_("Modified Date :"),sql2date($myrow['modified_date_time']));
		label_row(_("Modified User :"),$modified_user_data['user_id']);
		}
	}	
