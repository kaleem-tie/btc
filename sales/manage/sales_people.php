...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if (strlen($_POST['salesman_abrev']) == 0) 
	{
		$input_error = 1;
		display_error( _("The sales person abbreviation must be entered."));
		set_focus('salesman_abrev');
		return false;
	}
	
	if($Mode=='ADD_ITEM')
     {
	$valid_salesman_abrev = getvalid_salesman_abrev(trim($_POST['salesman_abrev']));
		
		 if(!empty(trim($valid_salesman_abrev))){
		   $input_error = 1;
		   display_error(_("Sales person abbreviation should be unique."));
		   set_focus('salesman_abrev');
		   return false;
		}
     }
	...
	if($Mode=='UPDATE_ITEM')
    {
	$valid_salesman_abrev_edit = getvalid_salesman_abrev_edit(trim($_POST['salesman_abrev']),$selected_id);
		
		 if(!empty(trim($valid_salesman_abrev_edit))){
		   $input_error = 1;
		   display_error(_("Sales person abbreviation should be unique."));
		   set_focus('salesman_abrev');
		   return false;
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
			...
			update_salesman($selected_id, $_POST['salesman_name'], $_POST['salesman_phone'], $_POST['salesman_fax'],$_POST['salesman_email'], input_num('provision'),
			input_num('break_pt'), input_num('provision2'),$_POST['salesman_abrev']);
    	}
    	else
    	{
    		/*Selected group is null cos no item selected on first time round so must be adding a record must be submitting new entries in the new Sales-person form */
			add_salesman($_POST['salesman_name'], $_POST['salesman_phone'], $_POST['salesman_fax'],$_POST['salesman_email'], input_num('provision'), input_num('break_pt'), 
			input_num('provision2'),$_POST['salesman_abrev']);
    	}
...
if ($Mode == 'Delete')
{
	...
	{
		if (key_in_foreign_table($selected_id, 'debtor_trans', 'sales_person_id'))
		{
			display_error(_("Cannot delete this sales-person because it is used by some sales transactions."));
		}
		else
		{
...
$th = array(_("Abbreviation"), _("Name"), _("Phone"), _("Fax"), _("Email"), _("Provision"), _("Break Pt."), _("Provision")." 2", "", "");
...
while ($myrow = db_fetch($result))
{
    ...
    label_cell($myrow["salesman_abrev"]);
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['salesman_abrev'] = $myrow["salesman_abrev"];
	}
...
text_row_ex(_("Sales Person Abbreviation:"), 'salesman_abrev', 1, 1);
