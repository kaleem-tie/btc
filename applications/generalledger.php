...
class general_ledger_app extends application
{
	function __construct()
	{
		...
		$this->add_rapp_function(0, _("&Multiple Purchase Cash Bill Entry"),
			"gl/multi_cash_bill.php?NewPayment=Yes", 'SA_MULTI_CASH_BILL', MENU_TRANSACTION);

        $this->add_lapp_function(0, _("&Petty Cash Entry"),
			"gl/gl_petty_cash_entry.php?NewPettyCash=Yes", 'SA_PETTY_CASH_ENTRY', MENU_TRANSACTION);

		$this->add_lapp_function(0, _("Purchase &Journal"),
			"purchasing/purchase_journal.php?NewInvoice=Yes", 'SA_SUPPLIERINVOICE', MENU_TRANSACTION);			
		...
		$this->add_lapp_function(1, _("&VAT Inquiry"),
			"gl/inquiry/vat_inquiry.php?", 'SA_GLTRANSVIEW', MENU_INQUIRY);	
			
		$this->add_lapp_function(1, _("Petty Cash Inquiry"),
			"gl/inquiry/gl_petty_cash_inquiry.php?", 'SA_PETTY_CASH_INQ_VIEW', MENU_INQUIRY);	
