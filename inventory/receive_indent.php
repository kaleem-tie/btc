...
***********************************************************************/
$page_security = 'SA_RECEIVE_INDENT';
$path_to_root = "..";
...
include_once($path_to_root . "/inventory/includes/stock_receive_ui.inc");
...
// if (isset($_GET['NewTransfer'])) {
	...
	else if (isset($_GET['NewReceive'])) {
	$_SESSION['NewTransfer'] = '';
	$help_context = 'Inventory Receive Against Indent';
	$_SESSION['page_title'] = sprintf( _("Inventory Receive Against Indent"), $_GET['ModifyPurchIndent']);
	
	
	
	if(list_updated('receiver_id')){
	$trans_no=get_transer_id($_POST['receiver_id']);
	 $_SESSION['transfer_items'] = new items_cart(ST_INDENT_REC,$trans_no);
	}
...
if(list_updated('mi_id'))
{
	$Ajax->activate('_page_body');
}
if (list_updated('FromStockLocation') ||  list_updated('ToStockLocation'))
	{
	$Ajax->activate('_page_body');
	} 
...
if (isset($_GET['AddedID'])) 
{
	...
	//display_note(get_gl_view_str(ST_LOCTRANSFER, $trans_no, _("View the GL &Postings for this Transfer")), 1, 0);
...
if (isset($_POST['Process']))
{
	...
	if($_POST['receiver_id']=='-1')
	{
			display_error(_("Please select the material receive indents"));
			set_focus('receiver_id');
			$input_error = 1;
	}
	if ($input_error == 1)
		unset($_POST['Process']);
...
if (isset($_POST['Process']))
{
	$transfer=get_transer_details($_POST['receiver_id']);
	$trans_no = add_stock_received($_SESSION['transfer_items']->line_items,
		$_POST['FromStockLocation'], $_POST['ToStockLocation'],
		$_POST['AdjDate'], $transfer['reference'], $_POST['memo_'],$_POST['receiver_id'],$transfer['transfer_id']);
...
function check_item_data()
{
	...
	if($_POST['tot']==2)
	{
		if($indent_id =='-1')
		{
			display_error(_("Please select the indent"));
			set_focus('indent_id');
			return false;
		}
		if(strlen($_POST['stock_id'])=='')
		{
			display_error(_("Please select the item"));
			set_focus('stock_id');
			return false;
		}
		$leftover_qty=get_mir_total_qty_itemwise($_POST['indent_id'],$_POST['stock_id']);
		if(input_num('qty')>$leftover_qty)
		{
			display_error(_("The quantity entered is more then the requested quantity left over quantity is #".$leftover_qty));
			set_focus('qty');
			return false;
		}
	}
...
if (isset($_GET['NewReceive']) || !isset($_SESSION['transfer_items']))
{
