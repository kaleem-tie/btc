...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	if($Mode=='ADD_ITEM')
     {
		$valid_supplier_code = getvalid_supplier_code(trim($_POST['supplier_description']));
		 if(!empty(trim($valid_supplier_code))){
		   $input_error = 1;
		   display_error(_("Item supplier description should be unique."));
		   set_focus('supplier_description');
		   return false;
		}
     }
	  ...
	  if($Mode=='UPDATE_ITEM')
     {
		$valid_supplier_code_edit = getvalid_supplier_code_edit(trim($_POST['supplier_description']),$_POST['stock_id'],$_POST['supplier_id']);
		
		 if(!empty(trim($valid_supplier_code_edit))){
		   $input_error = 1;
		   display_error(_("Item supplier description should be unique."));
		   set_focus('supplier_description');
		   return false;
...
if(isset($_POST['stock_id']) && !list_updated('supplier_id'))
{
	$_POST['supplier_id']=get_item_supplier_id($_POST['stock_id']);
...
if (!$page_nested)
{
	...
	echo stock_items_list('stock_id', $_POST['stock_id'], true, true);
	echo "<hr></center>";
}
else
	br(2);
...
amount_row(_("Price:"), 'price', null,'', get_supplier_currency($_POST['supplier_id']), $dec2);
