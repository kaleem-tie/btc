...
include_once($path_to_root . "/inventory/includes/db/promotional_discounts_db.inc");
...
page("Offers - Categories", false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(_("Item Category should not be empty."));
	...
	if ($input_error != 1) 
	{
    	...
    	if ($selected_id != -1) 
    	{
    		update_promotional_discounts($selected_id, $category_id, $_POST['from_date'], $_POST['to_date'], input_num('disc'), $_POST['offer_name']);
			display_notification(_('Selected offers - categories has been updated'));
    	} 
    	else 
    	{
    		add_promotional_discounts($category_id, $_POST['from_date'], $_POST['to_date'], input_num('disc'), $_POST['offer_name']);
			display_notification(_('New offers - categories has been added'));
    	}
		$Mode = 'RESET';
	}
} 
...
if ($Mode == 'Delete')
{
		delete_promotional_discount($selected_id);
...
//-----------------------------------------------------------------------------------
 ...
 $result = get_promotional_discounts(check_value('show_inactive'));

start_form();
start_table(TABLESTYLE, "width='50%'");
$th = array(_("Offer Name"),_("Item Category"), _("From Date"), _("To Date"),_("Discount %"), "", "");
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_promotional_discount($selected_id);
		
		// $_POST['category_id']  = $myrow["category_id"];
		
		$_POST['category_id']  = explode(',',$myrow["category_id"]);
...
stock_categories_list_row(_("Item Category:"), 'category_id', null, false, false,false,true);
