...
$sql ="SELECT (RIGHT(stock_id,8)) as stk_id FROM ".TB_PREF."stock_master WHERE RIGHT(stock_id,8) REGEXP '^-?[0-9]+$' order by stock_id desc limit 1";
	...
	if(strlen($result['0']) == '1'){
		...
		$stk_id = '10000000'.($result['0']);
		
	}
	else if(strlen($result['0']) == '2'){
		
		$stk_id = '1000000'.($result['0']);
		
	}
	else if(strlen($result['0']) == '3'){
		
		$stk_id = '100000'.($result['0']);
		
	}
	if(strlen($result['0']) == '4'){
		
		$stk_id = '10000'.($result['0']);
		
	}
	else if(strlen($result['0']) == '5'){
		
		$stk_id = '1000'.$result['0'];
	}
	else if(strlen($result['0']) == '6'){
		
		$stk_id = '100'.$result['0'];
	}
	else if(strlen($result['0']) == '7'){
...
if (isset($_POST['addupdate'])) 
{
	...
	/*
	if (!get_post('fixed_asset') &&  !is_service(get_post('mb_flag'))) {
	   if (strlen($_POST['arabic_description']) == 0) 
	  {
		$input_error = 1;
		display_error( _('The arabic description must be entered.'));
		set_focus('arabic_description');
	  } 
	}
	*/
	...
	if ($input_error != 1)
	{
		...
		{ /*so its an existing one */
			update_item($_POST['NewStockID'], $_POST['description'],
				...
				get_post('fa_class_id'), $_POST["supplier_id"], input_num('length'), input_num('breadth'), input_num('height'), input_num('cbf'), $_POST['packing_details'],$_POST['old_code'],$_POST['replacement_code'],$_POST['hs_code'],input_num('weight'),input_num('lead_days'),$_POST['country_id'],$_POST['item_sub_category'],$_POST['arabic_description'],$_POST['supplier_item_code'], get_post('sec_units'),input_num('conversion_factor'));
		...
		{ //it is a NEW part
			...
			$_POST['description'] = $_POST['long_description'] = $_POST['arabic_description'] ='';
			$_POST['no_sale'] = $_POST['editable'] = $_POST['no_purchase'] = $_POST['supplier_id'] =0;
			set_focus('NewStockID');
		}
		$Ajax->activate('_page_body');
	}
...
function item_settings(&$stock_id, $new_item) 
{
	...
	if ($new_item) 
	{
		...
		/*
			$tmpCodeID=create_item_code();				
			$_POST['NewStockID'] = $tmpCodeID;
		label_row(_("Item Code:"), $tmpCodeID,"class='label'");
		hidden('NewStockID',$tmpCodeID);
		$_POST['inactive'] = 0;
		*/
	} 
	else 
	...
	text_row(_("Name:<b style='color:red;'>*</b>"), 'description', null, 52, 200);
	if (!get_post('fixed_asset')) {
    text_row(_("Arabic Description:"), 'arabic_description', null, 52, 200);
	}
	textarea_row(_('Description:'), 'long_description', null, 42, 3);
	
	if (!get_post('fixed_asset')) {
	//supplier_list_row(_("Supplier:"), 'supplier_id', null, false, false);
	//text_row(_("Supplier Item Code:"), 'supplier_item_code', null, 15, 15);
	hidden('supplier_id', 0);
	hidden('supplier_item_code', "");
	}
	stock_categories_list_row(_("Category:"), 'category_id', null, _("Select a Category"), $new_item, $fixed_asset);
	...
	if ($new_item && (list_updated('category_id') || !isset($_POST['sales_account']))) { // changed category for new item or first page view
		...
		$_POST['sec_units'] = $category_record["dflt_units"];
  ...
  item_tax_types_list_row(_("Item Tax Type:"), 'tax_type_id', null, !$new_item && item_type_inactive(get_post('tax_type_id')));
	...
	stock_units_list_row(_('Primary Units of Measure:'), 'units', null, $fresh_item);
    ...
    amount_row(_("Conversion Factor (to our Primary UOM):"), 'conversion_factor', null, null, null, 'max',!$fresh_item);

    stock_units_list_row(_('Secondary Units of Measure:'), 'sec_units', null, $fresh_item);
	...
	if (!get_post('fixed_asset')) {
	table_section(2);
		table_section_title(_("Customized"));
		...
		text_row(_('No of Packs:'), 'packing_details', 1, 15, 4);
	}
