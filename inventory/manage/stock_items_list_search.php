...
if(!isset($dataval)){
	 ...
	 */
	  ...
	  $sql = "SELECT s.stock_id, CONCAT(s.stock_id,'       ',s.description) as item_desc, c.description as category_description, s.inactive, s.editable FROM ".TB_PREF."stock_master s
       RIGHT OUTER JOIN  ".TB_PREF."purch_data purch
	   ON purch.stock_id=s.stock_id,
      ".TB_PREF."stock_category c WHERE s.category_id=c.category_id  AND mb_flag!='F' AND (s.stock_id LIKE '%".$search."%' 
       or  s.description LIKE '%".$search."%' or  purch.supplier_description LIKE '%".$search."%') GROUP BY s.stock_id order by c.description, s.stock_id  LIMIT 0,200";
	// echo $sql;die;
