...
include_once($path_to_root . "/inventory/includes/db/promotional_discounts_brands_db.inc");
...
page("Offers - Suppliers", false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(_("supplier should not be empty.!"));
	...
	if ($input_error != 1) 
	{
		$supplier_id=implode(',',$_POST["supplier_id"]);
		
    	if ($selected_id != -1) 
    	{
    		update_brands_promotional_discounts($selected_id, $supplier_id, $_POST['from_date'], $_POST['to_date'], input_num('disc'), $_POST['offer_name']);
			display_notification(_('Selected offers - brands has been updated'));
    	} 
    	else 
    	{
    		add_brands_promotional_discounts($supplier_id, $_POST['from_date'], $_POST['to_date'], input_num('disc'), $_POST['offer_name']);
			display_notification(_('New offers - brands has been added'));
    	}
		$Mode = 'RESET';
	}
} 
...
if ($Mode == 'Delete')
{
		delete_brands_promotional_discount($selected_id);
		display_notification(_('Selected offers-brands has been deleted'));
	$Mode = 'RESET';
}
...
//-----------------------------------------------------------------------------------
 ...
 $result = get_all_brands_promotional_discounts(check_value('show_inactive'));

start_form();
start_table(TABLESTYLE, "width='50%'");
$th = array(_("Offer Name"),_("Brands"), _("From Date"), _("To Date"),_("Discount %"), "", "");
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell($myrow["supplier_name"]);
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$myrow = get_brands_promotional_discount($selected_id);
		
		$_POST['supplier_id']  = explode(',',$myrow["supplier_id"]);
...
supplier_list_row(_("Supplier:"), 'supplier_id', null, false, false,false,false,true);
