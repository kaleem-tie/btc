...
{
	$th = array(_("Location"), _("Qty On Hand"),  _("Primary UOM"),
         _("Qty On Hand"),  _("Secondary UOM"),	_("Re-Order Level"),
		_("Demand"), _("Available"), _("In Order"));
}
table_header($th);
...
while ($myrow = db_fetch($loc_details))
{
	...
	if ($kitset_or_service == false)
	{
		...
		if($sec_unit_info["conversion_factor"]!=0)
		$sec_qoh = ($qoh)*$sec_unit_info["conversion_factor"];
		else
		$sec_qoh = 0;	
		
		qty_cell($sec_qoh, false, 2);
