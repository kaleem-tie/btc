...
include_once($path_to_root . "/gl/includes/ui/multi_cash_bill_ui.inc");
...
if (isset($_GET['NewPayment'])) {
	$_SESSION['page_title'] = _($help_context = "Multiple Purchase Cash Bill Entry");
...
if (isset($_GET['AddedID']))
{
	...
	display_note(get_trans_view_str($trans_type, $trans_no, _("&View this Payment")), 0, 1);
	
	//display_note(get_trans_view_str($trans_type,$trans_no, _("&View This Payment")));
	...
	hyperlink_params("$path_to_root/purchasing/allocations/supplier_allocation_main.php?", _("Enter Supplier Allocations"));
...
if (isset($_GET['AddedDep']))
{
	...
	hyperlink_params("$path_to_root/sales/allocations/customer_allocation_main.php?", _("Enter Customer Allocations"));
...
function check_trans()
{
	...
	//ravi
	if (!check_reference($_POST['ref'], $_SESSION['pay_items']->trans_type, $_SESSION['pay_items']->order_id,null,null,$_POST['bank_account']))
	{
		set_focus('ref');
		$input_error = 1;
	}
...
if (isset($_POST['Process']) && !check_trans())
{
	...
	$trans = write_bank_transaction(
		...
		$_POST['mode_of_payment'],$_POST['cheque_no'],$_POST['dd_no'],$_POST['date_of_issue'],
		$_POST['dd_date_of_issue'],$_POST['pymt_ref'],$is_purch_cash_bill,$_POST['our_ref_no'],$ex_rate,check_value('amex')); //ravi
...
function check_item_data()
{
	...
	if (!check_num('bill_amount', 0))
	{
		display_error( _("The bill amount entered is not a valid number or is less than zero."));
		set_focus('bill_amount');
		return false;
	}
	if (strlen($_POST['supp_name'])==0)
	{
		display_error( _("Supplier name can not be empty"));
		set_focus('supp_name');
		return false;
	}
	if (strlen($_POST['supp_bill_no'])==0)
	{
		display_error( _("Bill number can not be empty"));
		set_focus('supp_bill_no');
		return false;
	}
	if (!is_date($_POST['supp_bill_date'])) 
	{
		display_error(_("The entered date for the bill date is invalid."));
		set_focus('supp_bill_date');
		return false;
	} 
	elseif (!is_date_in_fiscalyear($_POST['supp_bill_date'])) 
	{
		display_error(_("The entered bill date is out of fiscal year or is closed for further data entry."));
		set_focus('supp_bill_date');
		return false;
	}
...
function handle_update_item()
{
    ...
    if($_POST['UpdateItem'] != "" && check_item_data())
    {
    	$_SESSION['pay_items']->update_gl_item($_POST['Index'], $_POST['code_id'], 
    	    $_POST['dimension_id'], $_POST['dimension2_id'], $amount , $_POST['LineMemo'],'',0,$_POST['supp_name'],$_POST['supp_vat_no'],$_POST['supp_bill_no'],$_POST['supp_bill_date'],input_num('bill_amount'),check_value('is_tax_account'));
    }
	line_start_focus();
}
...
function handle_new_item()
{
	...
	$_SESSION['pay_items']->add_gl_item($_POST['code_id'], $_POST['dimension_id'],
		$_POST['dimension2_id'], $amount, $_POST['LineMemo'],'',0,'',$_POST['supp_name'],$_POST['supp_vat_no'],$_POST['supp_bill_no'],$_POST['supp_bill_date'],input_num('bill_amount'),check_value('is_tax_account'));
	line_start_focus();
}
//-----------------------------------------------------------------------------------------------
