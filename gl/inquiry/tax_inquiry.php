...
function show_results()
{
	...
	// get multiple purchase cash bills tax data_checks
	
	$input_vat_multiple_cash_bills = get_input_vat_on_multiple_cash_bills($_POST['TransFromDate'], $_POST['TransToDate']);
	
	alt_table_row_color($k);
	label_cell("<b>INPUTTAX 5% % </b>");
	label_cell("<b>"._("VAT on Multiple Cash Bill (Input Tax)") . ":</b>");
	amount_cell(-$input_vat_multiple_cash_bills, true);
	label_cell("");
	end_row();	
	$total -= $input_vat_multiple_cash_bills;
	//end
	
	
	
	// get 1708 input VAT import 5%
	
	$input_vat_import=get_input_vat_import($_POST['TransFromDate'], $_POST['TransToDate']);
	alt_table_row_color($k);
	label_cell("<b>Input VAT on Import @ 5 % </b>");
	label_cell("<b>"._("VAT on Import (Input Tax)") . ":</b>");
	amount_cell(-$input_vat_import, true);
	label_cell("");
	end_row();	
	$total -= $input_vat_import;
	//end
	alt_table_row_color($k);
	label_cell("");
