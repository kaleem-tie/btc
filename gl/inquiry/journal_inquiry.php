...
function edit_link($row)
{
	...
	$is_it_pettycash   = is_it_gl_pettycash_trans($row["trans_no"]);
	
	if($row['trans_type'] == ST_BANKPAYMENT && $is_it_pettycash>0){
		return "";
	}
	else{
...
$cheq = get_cheque_print_gl($row['trans_no'], $row['trans_type']);
	...
	if ($row['trans_type'] == ST_BANKTRANSFER)
	{
		
 		return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print Transfer Format"), true, ST_BANKTRANSFER, ICON_PRINT);
   	}elseif ($row['trans_type'] == ST_BANKPAYMENT)
	{
	...
	}elseif ($row['trans_type'] == ST_BANKDEPOSIT)
	{
 		...
 		return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print Remittance Format"), true, ST_BANKDEPOSIT, ICON_PRINT);
	}elseif ($row['trans_type'] == ST_SUPPAYMENT) { 
		return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print Remittance Format"), true, ST_SUPPAYMENT_REP, ICON_PRINT);
	}
...
function chq_prt_link($row)
{
	...
	if ($row['trans_type'] == ST_BANKTRANSFER)
	{
 		...
 		return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print Transfer Format"), true, ST_BANKTRANSFER_REP, ICON_PRINT);
	} if ($row['trans_type'] == ST_SUPPAYMENT) {
		return print_document_link($row['trans_no']."-".$row['trans_type'], _("Print Remittance Format"), true, ST_SUPPAYMENT_REP_TWO, ICON_PRINT);
		}
...
$cols = array(
	...
	_("Cheque Print") => array('insert'=>true, 'fun'=>'chq_prt_link')
);
