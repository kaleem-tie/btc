...
if (isset($_GET['AddedID']))
{
	...
	$bank_payment_mode = get_cheque_print_gl($trans_no, $trans_type);
	if($bank_payment_mode == "cheque"){
	display_note(print_document_link($trans_no, _("&Print Cheque Payment"), true, ST_BANKPAYMENT_REP), 0, 1);
	br(1);	
	submenu_option(_("Payment Cheque Transaction Update"), "/purchasing/inquiry/bank_cheque_transaction_update.php?trans_type=".$trans_type."&trans_no=".$trans_no);
	}	
	else{
	display_note(print_document_link($trans_no, _("&Print Payment"), true, $trans_type), 0, 1);
	}
...
if (isset($_GET['AddedDep']))
{
	...
	display_note(print_document_link($trans_no, _("&Print Deposit"), true, $trans_type), 0, 1);
...
function check_trans()
{
	...
	/*
	if ($_POST['code_id'] == '') {
		display_error(_("Select a Account Name cannot be empty."));
		set_focus('code_id');
		$input_error = 1;
	}*/
	...
	*/
	...
	if($_POST['mode_of_payment'] == 'cheque')
	{
		...
		if($_SESSION['pay_items']->trans_type==ST_BANKPAYMENT && $_SESSION['pay_items']->order_id==0){
		$cheque_no_exist = check_exists_bank_payment_cheque_no(trim($_POST['cheque_no']), $_POST['trans_no'], ST_BANKPAYMENT);
	      if($cheque_no_exist!=0 && $_POST['trans_no']==0){
		   
		   $cust_cheque_no = get_exists_bank_payment_cheque_no(trim($_POST['cheque_no']), ST_BANKPAYMENT);
...
if (isset($_POST['Process']) && !check_trans())
{
	...
	$trans = write_bank_transaction_for_multiple_entry(
		$_SESSION['pay_items']->trans_type, $_SESSION['pay_items']->order_id, $_POST['bank_account'],
		$_SESSION['pay_items'], $_POST['date_'],$_POST['PayType'],
		...
		$_POST['dd_date_of_issue'],$_POST['pymt_ref'],0,$_POST['our_ref_no'],$ex_rate,
		check_value('amex'));
...
function check_item_data()
{
	...
	if (is_subledger_account(get_post('code_id'))) {
		if(!get_post('person_id')) {
	   		display_error(_("You must select a party."));
   			$Ajax->activate('items_table');
			set_focus('person_id');
	   		return false;
	   	}
