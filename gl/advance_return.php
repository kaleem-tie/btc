...
$page_security = isset($_GET['NewPayment']) || 
 ...
 ? 'SA_SALES_ADVNC_RTN_PAYMNT' : 'SA_DEPOSIT';
...
function create_cart($type, $trans_no)
{
	...
	if ($trans_no) {
		...
		$_POST['ref'] = $bank_trans["reference"];
...
function check_trans()
{
	...
	if ($_POST['bank_account'] == '') {
		display_error(_("Bank Account cannot be empty."));
	...
	//check the advance price with total value
	$advances_info=get_sales_advances_info($_POST['person_id']);
    $allocated_adv_amount=0;	   	   
	while($sales_advance=db_fetch($advances_info))
	{         
		          if(check_value('adv_'.$sales_advance['trans_no'])==1)
				  { 
				    $allocated_adv_amount+=$sales_advance['ov_amount'];
				  }
	}
	if($allocated_adv_amount<$_SESSION['pay_items']->gl_items_total())
	{
		$input_error = 1;
		display_error("Entered Ledger amount should not exceed selected advances amount!");
	}
    // end
...
if (isset($_POST['Process']) && !check_trans())
{
	...
	$trans = write_bank_transaction_for_advance_return(
		...
		$_POST['dd_date_of_issue'],$_POST['pymt_ref'],0,$_POST['our_ref_no'],$ex_rate,check_value('amex'),$advance_ids);
...
display_bank_header_advance_return($_SESSION['pay_items']);
