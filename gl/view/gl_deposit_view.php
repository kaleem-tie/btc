...
start_row();
label_cells(_("Mode Of Payment"), $mode_payment_types[$to_trans['mode_of_payment']], "class='tableheader2'");
if($to_trans['mode_of_payment']=='cheque')
{
	label_cells(_("Cheque No"), $to_trans['cheque_no'], "class='tableheader2'");
	label_cells(_("Date Of Issue"), sql2date($to_trans['date_of_issue']), "class='tableheader2'");
}else if($to_trans['mode_of_payment']=='dd'){
	label_cells(_("DD No"), $to_trans['dd_no'], "class='tableheader2'");
	label_cells(_("Date Of Issue"), sql2date($to_trans['dd_date_of_issue']), "class='tableheader2'");
}
 else if($to_trans['mode_of_payment'] == 'ot' || $to_trans['mode_of_payment'] == 'neft' || $to_trans['mode_of_payment'] == 'card' || $to_trans['mode_of_payment'] == 'rtgs'){
	 
	if($to_trans['mode_of_payment'] == 'card'){
	label_cells(_("Card Last 4 Digits"), $to_trans['pymt_ref'], "class='tableheader2'");
	}
...
$preared_user = get_transaction_prepared_by(ST_BANKDEPOSIT, $trans_no);
