...
function get_all_bank_accounts_info()
{
	$sql = "SELECT id,bank_account_name FROM ".TB_PREF."bank_accounts 
    WHERE inactive=0";
    return db_query($sql,"No transactions were returned");
}
...
function get_sql_for_bank_payment_from_reference($filtertype, $from, $to,$bank_account)
{
  ...
  WHERE trans_date between ".db_escape($fromdate)." and ".db_escape($todate)." 
  AND ref!='auto' AND type=1 AND bank_act = ".db_escape($bank_account)." ORDER BY trans_date ASC LIMIT 1";
...
function get_sql_for_bank_payment_to_reference($filtertype, $from, $to,$bank_account)
{
  ...
  $sql = " SELECT ref  FROM ".TB_PREF."bank_trans 
  WHERE trans_date between ".db_escape($fromdate)." and ".db_escape($todate)." 
  AND ref!='auto' AND type=1  AND bank_act = ".db_escape($bank_account)." ORDER BY trans_date DESC LIMIT 1";
...
function get_sql_for_bank_deposit_from_reference($filtertype, $from, $to,$bank_account)
{
  ...
  $sql = " SELECT ref  FROM ".TB_PREF."bank_trans 
  WHERE trans_date between ".db_escape($fromdate)." and ".db_escape($todate)." 
  AND ref!='auto' AND type=2 AND bank_act = ".db_escape($bank_account)." ORDER BY trans_date ASC LIMIT 1";
...
function get_sql_for_bank_deposit_to_reference($filtertype, $from, $to,$bank_account)
{
  ...
  $sql = " SELECT ref  FROM ".TB_PREF."bank_trans 
  WHERE trans_date between ".db_escape($fromdate)." and ".db_escape($todate)." 
  AND ref!='auto' AND type=2  AND bank_act = ".db_escape($bank_account)." ORDER BY trans_date DESC LIMIT 1";
...
function print_series_of_documents()
{
    ...
    $cols = array(0, 100,  330, 450, 530);

    $headers = array(_('Transaction Type'),_(''), _('From Reference'), _('To Reference'));
    ...
    while ($myrow=db_fetch($trans))
    {
     ...
     if (!in_array($myrow['trans_type'], array(ST_SUPPRECEIVE,ST_DIMENSION,
     ST_INVSUPPRECEIVE,ST_BANKPAYMENT,ST_BANKDEPOSIT))){
    ...
    $to_reference = get_sql_for_trans_type_to_reference($myrow['trans_type'], $from, $to);
     ...
     $to_reference = get_sql_for_inventory_grn_to_reference($myrow['trans_type'], $from, $to);
        ...
        if($myrow['trans_type']!=ST_BANKPAYMENT && $myrow['trans_type']!=ST_BANKDEPOSIT ){
        $rep->TextCol(2, 3, $from_reference);
        $rep->TextCol(3, 4, $to_reference);
        $rep->NewLine();
        }
        

        else if($myrow['trans_type']==ST_BANKPAYMENT){
          $rep->NewLine();
          $bank_accounts = get_all_bank_accounts_info();
           while ($bank_acc=db_fetch($bank_accounts)){

        
        $bp_from_reference = get_sql_for_bank_payment_from_reference($myrow['trans_type'], $from, $to,$bank_acc['id']);
        $bp_to_reference = get_sql_for_bank_payment_to_reference($myrow['trans_type'], $from, $to,$bank_acc['id']);
     
           //display_error($bank_acc['bank_account_name']. " - ".$bp_from_reference. " - ".$bp_to_reference);
      
           if($bp_from_reference!=0 &&  $bp_to_reference!=0){
           $rep->TextCol(1, 2, $bank_acc['bank_account_name']);
           $rep->TextCol(2, 3, $bp_from_reference);
           $rep->TextCol(3, 4, $bp_to_reference);
           $rep->NewLine();
           }
          }
         $rep->NewLine();
        ...
        else if($myrow['trans_type']==ST_BANKDEPOSIT){
        $rep->NewLine();
        $bank_accounts = get_all_bank_accounts_info();
        while ($bank_acc=db_fetch($bank_accounts)){

        
        $bd_from_reference = get_sql_for_bank_deposit_from_reference($myrow['trans_type'], $from, $to,$bank_acc['id']);
        $bd_to_reference = get_sql_for_bank_deposit_to_reference($myrow['trans_type'], $from, $to,$bank_acc['id']);
     
           //display_error($bank_acc['bank_account_name']. " - ".$bp_from_reference. " - ".$bp_to_reference);
      
           if($bd_from_reference!=0 &&  $bd_to_reference!=0){
           $rep->TextCol(1, 2, $bank_acc['bank_account_name']);
           $rep->TextCol(2, 3, $bd_from_reference);
           $rep->TextCol(3, 4, $bd_to_reference);
           $rep->NewLine();
           }
          }
         $rep->NewLine();
