...
function get_invoices($customer_id, $to, $all=true, $folk=0)
{
	...
	$sign = "IF(`type` IN(".implode(',',  array(ST_CUSTCREDIT,ST_CUSTPAYMENT,ST_BANKDEPOSIT))."), -1, IF(type=".ST_JOURNAL." AND trans.ov_amount<0,-1,1))";
...
function print_aged_customer_analysis()
{
	...
	$headers = array(_('Customer'),	'',	'',	_('Current'), $nowdue, $pastdue1, $pastdue2,$pastdue3,_('Total Balance'));
	...
	$sql = "SELECT m.debtor_no, m.name, m.curr_code FROM ".TB_PREF."debtors_master m, ".TB_PREF."debtor_trans t WHERE t.debtor_no=m.debtor_no ";
	if ($fromcust != ALL_TEXT){
	...
	while ($myrow=db_fetch($result))
	{
		...
		$total[4] += $custrec["Overdue3"];
		$total[5] += $custrec["Balance"];
		for ($i = 0; $i < count($str); $i++)
			$rep->AmountCol($i + 3, $i + 4, $str[$i], $dec);
		$rep->NewLine(1, 2);
		if (!$summaryOnly)
		{
			$res = get_invoices($myrow['debtor_no'], $to, $show_all, $folk);
