...
//----------------------------------------------------------------------------------------------------
function get_purch_enquiry($order_no)
{
...
function get_purch_enquiry_details($order_no)
{
	$sql = "SELECT poline.*, units
		...
		WHERE order_no =".db_escape($order_no)." AND poline.trans_type =".ST_PURCHENQ." ";
...
function print_po()
{
	...
	$cols = array(2,40, 100, 350, 470, 520);

	// $headers in doctext.inc
	$aligns = array('left',	'left', 'left',	'right', 'center');
	...
	if ($email == 0)
		$rep = new FrontReport(_('PURCHASE ENQUIRY'), "PurchaseEnquiryBulk", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
    	recalculate_cols($cols);

	for ($i = $from; $i <= $to; $i++)
	{
		$myrow = get_purch_enquiry($i);
		...
		if ($email == 1)
		{
			...
			$rep->title = _('PURCHASE ENQUIRY');
			$rep->filename = "PurchaseEnquiry" . $i . ".pdf";
		}	
		$rep->currency = $cur;
		$rep->Font();
		...
		$rep->SetCommonData($myrow, null, $myrow, $baccount, ST_PURCHENQ, $contacts);
		$rep->SetHeaderType('Header22');
		$rep->NewPage();

		$result = get_purch_enquiry_details($i);
		$SubTotal = 0;
		$items = $prices = array();
		$k=1;
		while ($myrow2=db_fetch($result))
		{
			...
			$DisplayQty = number_format2($myrow2["quantity_ordered"],3);
       ...
       /*
		...
		$rep->Font('bold');
		$rep->TextCol(3, 6, _("TOTAL ENQUIRY"), - 2);
		$rep->TextCol(6, 7,	$DisplayTotal, -2);
		$words = price_in_words($SubTotal, ST_PURCHENQ);
		if ($words != "")
		{
			$rep->NewLine(1);
