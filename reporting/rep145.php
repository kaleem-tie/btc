...
***********************************************************************/
$page_security = 'SA_SALES_RETURN_REG_REP';
...
function get_sales_credit_note_line_transactions($invoice_no) {
	...
	$sql = "SELECT 
		...
		AND trans.type = 11 and line.debtor_trans_type = 11
		AND item.mb_flag <>'F'
...
function print_sales_return_register()
{
    ...
    $rep = new FrontReport(_('Sales Return Register'), 
	"SalesReturnRegister", user_pagesize(), 7, $orientation);
	...
	while ($myrow = db_fetch($result))
	{
		  ...
		  if (!$summaryOnly)
		  {
			...
			$inv_details = get_sales_credit_note_line_transactions($myrow['trans_no']);
