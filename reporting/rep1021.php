...
function print_customer_orders()
{
	...
	$cols = array(0, 20, 65, 180,220,250,290,330,380,410,450,500,590);

	$headers = array(_('S.No'), _('Date'), _('Customer'), _('Reference'),  
	_('Currency'),_('Rate'),_('Alloc Amount'), 
	_('Taxable Value(in OMR)'),_('Tax Value'),_('Round off'), _('Total Amount'), _('Comments'));

	
	$aligns = array('left','left','left','left','left',	'center','right','right','right','right','right', 'center');
    ...
    $params =   array( 	0 => $comments,
						...
						5 => array('text' => _('Sales Folk'), 'from' => $salesfolk,	'to' => ''));

    $rep = new FrontReport(_('Customer Credit Notes Listing'), "CustomerCreditnotesListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
$total_roundoff=0;
	...
	while ($myrow = db_fetch($result))
	{
		...
		$rep->TextCol(11, 12,	$com_res['memo_']);	
	...
	$rep->AmountCol(9, 10, $total_roundoff,$dec);
	$rep->AmountCol(10, 11, $tot_value,$dec);
    $rep->End();
}
