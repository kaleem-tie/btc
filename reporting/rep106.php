...
function GetSalesmanTrans($from, $to,$sales_person=0)
{
	...
	$sql = "SELECT DISTINCT trans.*,
			ov_amount+ov_discount+ov_roundoff AS InvoiceTotal,
	...
	$sql .= " AND trans.sales_person_id=".db_escape($sales_person);	
		...
		$sql .= " ORDER BY salesman.salesman_code, trans.tran_date";
...
function print_salesman_list()
{
	...
	$headers = array(_('Invoice'), _('Customer'), _(''), _('Customer Ref'),
	...
	$rep = new FrontReport(_('Salesman Listing (Invoices)'), "SalesmanListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
