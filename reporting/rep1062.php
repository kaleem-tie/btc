...
function get_transactions($customer_id=null,$from,$to,$location,$dimension=0, $folk=0)
{
	...
	AND ((trans.type=".ST_SALESINVOICE.") AND (trans.ov_amount > 0)) AND trans.signed_collection_status=1";
...
function get_collected_information($trans_no)
{	
	$sql ="SELECT collected_date,collected_by FROM ".TB_PREF."signed_copy_collection_register WHERE invoice_no=".db_escape($trans_no)."";
...
function print_customer_orders()
{
	...
	//$cols = array(0, 20, 65, 210,250,280,320,380,420,470,520,580);
	$cols = array(0, 20, 65, 170,210,240,280,340,380,420, 470,520,580);
      

	$headers = array(_('S.No'), _('Date'), _('Customer'), _('Reference'),  
	_('Currency'),_('Rate'), 
	_('Taxable Value(in OMR)'),_('Tax Value'), _('Roundoff'), _('Total Amount'),_('Collected Date'),_('Collected By'));

	
	$aligns = array('left','left','left','left','left',	'center','right','right','right','right','center','left');
    ...
    $rep = new FrontReport(_('Signed Invoice Collection Register'), "SignedInvoiceCollectionRegister", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
$total_roundoff=0;
	...
	while ($myrow = db_fetch($result))
	{
		...
		if($myrow['tax_included']==1)
		{
		 ...
		 $Total_tax=0;
		  $tax_items = get_trans_tax_details(ST_SALESINVOICE, $myrow['trans_no']);
			...
			$rep->AmountCol(6,7, $Total_amt*$myrow['rate'],$dec);
			$rep->AmountCol(7,8, $Total_tax*$myrow['rate'],$dec);
		
			$total_taxable_value+=$Total_amt*$myrow['rate'];
			$total_tax_value+=$Total_tax*$myrow['rate'];
			$total_roundoff+=$myrow['ov_roundoff'];
		} else {
			$rep->AmountCol(6,7, $myrow['Total']*$myrow['rate'],$dec);
			$rep->AmountCol(7,8, $myrow['ov_gst']*$myrow['rate'],$dec);
			$rep->AmountCol(8,9, $myrow['ov_roundoff'],$dec);
		...
		$rep->AmountCol(9,10, $myrow['Total']+$myrow['ov_gst']+$myrow['ov_roundoff'],$dec);
		  $copy_collection_details = get_collected_information($myrow['trans_no']);
		 $rep->TextCol(10, 11, sql2date($copy_collection_details['collected_date']));
		 $rep->TextCol(11, 12, $copy_collection_details['collected_by']);	
   		$rep->NewLine(1);
			$k++;
	}
	$rep->Line($rep->row  - 4);
	...
	$rep->AmountCol(6, 7, $total_taxable_value,$dec);
	$rep->AmountCol(7, 8, $total_tax_value,$dec);
	$rep->AmountCol(8, 9, $total_roundoff,$dec);
