...
function get_transactions($customer_id=null,$from,$to,$location,$dimension=0, $folk=0)
{
	...
	AND ((trans.type=".ST_SALESINVOICE.") AND (trans.ov_amount > 0)) AND trans.signed_collection_status=0";
...
function print_customer_orders()
{
	...
	//$cols = array(0, 20, 65, 210,250,280,320,390,470,520);
	//$cols = array(0, 20, 65, 210,250,280,320,355,430,470,520);
	$cols = array(0, 20, 65, 180,250,280,320,355,400,440,500,550);

	$headers = array(_('S.No'), _('Date'), _('Customer'), _('Reference'),  
	_('Currency'),_('Rate'), 
	_('Taxable Value(in OMR)'),_('Tax Value'), _('Roundoff'), _('Total Amount'), _('Comments'));

	
	$aligns = array('left','left','left','left','left',	'center','right','right','right','right', 'center');
    ...
    $rep = new FrontReport(_('Unsigned Invoices List'), "UnsignedInvoicesList", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
...
$total_roundoff=0;
	...
	// 	$rep->AmountCol(6, 7, $myrow['alloc'],$dec);
		...
		$rep->TextCol(10, 11,	$com_res['memo_']);	
				$rep->NewLine(1);
