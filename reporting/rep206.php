...
function get_open_balance($supplier_id, $to)
{
    ...
    $sql = "SELECT SUM(IF(t.type = ".ST_SUPPINVOICE." OR (t.type IN (".ST_JOURNAL." , ".ST_BANKDEPOSIT.") AND t.ov_amount>0),
        -abs(t.ov_amount + t.ov_gst + t.ov_discount+t.freight_cost+t.additional_charges+t.packing_charges+t.other_charges+t.freight_tax+t.additional_tax+t.packing_tax+t.other_tax+t.ov_roundoff), 0)) AS charges,";

 	$sql .= "SUM(IF(t.type != ".ST_SUPPINVOICE." AND NOT(t.type IN (".ST_JOURNAL." , ".ST_BANKDEPOSIT.") AND t.ov_amount>0),
        abs(t.ov_amount + t.ov_gst + t.ov_discount+t.freight_cost+t.additional_charges+t.packing_charges+t.other_charges+t.freight_tax+t.additional_tax+t.packing_tax+t.other_tax+t.ov_roundoff) * -1, 0)) AS credits,";
    ...
    $sql .= "SUM(IF(t.type = ".ST_SUPPINVOICE.", 1, -1) *
        (abs(t.ov_amount + t.ov_gst + t.ov_discount+t.freight_cost+t.additional_charges+t.packing_charges+t.other_charges+t.freight_tax+t.additional_tax+t.packing_tax+t.other_tax+t.ov_roundoff) - abs(t.alloc))) AS OutStanding
...
//ravi
function get_outstanding_balance($supplier_id, $to)
{
    if($to)
        $to = date2sql($to);
...
function getTransactions($supplier_id, $from, $to)
{
    ...
    $sql = "SELECT ".TB_PREF."supp_trans.*, comments.memo_,
        (".TB_PREF."supp_trans.ov_amount + ".TB_PREF."supp_trans.ov_gst + ".TB_PREF."supp_trans.ov_discount+ ".TB_PREF."supp_trans.freight_cost+".TB_PREF."supp_trans.additional_charges+".TB_PREF."supp_trans.packing_charges+".TB_PREF."supp_trans.other_charges+".TB_PREF."supp_trans.freight_tax+".TB_PREF."supp_trans.additional_tax+".TB_PREF."supp_trans.packing_tax+".TB_PREF."supp_trans.other_tax
		+".TB_PREF."supp_trans.ov_roundoff)
...
function print_supplier_balances()
{
    ...
    while ($myrow=db_fetch($result))
    {
		...
		//ravi
		$supp_outstanding_balance=get_outstanding_balance($myrow['supplier_id'], $to);
		$outstanding_db = $supp_outstanding_balance ? round2(abs($supp_outstanding_balance['charges'] * $rate), $dec) : 0; // db
        $outstanding_cr = $supp_outstanding_balance ? round2(abs($supp_outstanding_balance['credits'] * $rate), $dec) : 0; // cr
        $curr_outstanding = $outstanding_db-$outstanding_cr; 
		
		if($no_zeros && $curr_outstanding==0)
		continue;
        // end ravi	
