...
function print_po()
{
	...
	$cols = array(2, 30, 100, 230, 280, 330, 360, 400, 450, 520);

	// $headers in doctext.inc
	$aligns = array('left',	'left',	'left', 'left', 'right', 'left', 'right', 'right', 'right');
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$rep->TextCol(0, 1,	$sl_no, -2);
				...
				$oldrow = $rep->row;
				if($data !== false)
				$rep->TextColLines(2, 3, $myrow2['description'].' ('.	$data['supplier_description'].')', -2);
				else
