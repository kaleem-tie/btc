...
function getTransactions($debtorno, $date, $show_also_allocated, $folk=0)
{
    $sql = "SELECT trans.type,
        ...
        trans.tran_date,
        trans.due_date,
        IF(prep_amount, prep_amount, ov_amount + ov_gst + ov_freight + ov_freight_tax + ov_discount + ov_roundoff) AS TotalAmount,
        alloc AS Allocated,
        ...
        WHERE tran_date <= '$date' AND debtor_no = ".db_escape($debtorno)."
			...
			AND ABS(ABS(ov_amount) + ov_gst + ov_freight + ov_freight_tax + ov_discount + ov_roundoff) > ". FLOAT_COMP_DELTA;
	
  	if (!$show_also_allocated)
		$sql .= " AND ABS(IF(prep_amount, prep_amount, ABS(ov_amount) + ov_gst + ov_freight + ov_freight_tax + ov_discount + ov_roundoff) - alloc) > ". FLOAT_COMP_DELTA;
...
function print_statements()
{
	...
	while ($myrow=db_fetch($result))
	{
		...
		$TransResult = getTransactions($myrow['debtor_no'], $date, $show_also_allocated, $folk);
		...
		$str = array(_("Current"), $nowdue, $pastdue1, $pastdue2,$pastdue3, _("Total Balance"));
		$str2 = array(number_format2(($CustomerRecord["Balance"] - $CustomerRecord["Due"]),$dec),
			...
			number_format2(($CustomerRecord["Overdue2"]-$CustomerRecord["Overdue3"]) ,$dec),
			number_format2($CustomerRecord["Overdue3"],$dec),
			number_format2($CustomerRecord["Balance"],$dec));
		$col = array($rep->cols[0], $rep->cols[0] + 40, $rep->cols[0] + 140, 
		$rep->cols[0] + 220,$rep->cols[0] + 320, $rep->cols[0] + 420);
		$rep->row = $rep->bottomMargin + (19 * $rep->lineHeight - 6);
		for ($i = 0; $i < 6; $i++)
