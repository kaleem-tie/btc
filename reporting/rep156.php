...
{
	$sql ="SELECT cust_code FROM ".TB_PREF."debtors_master WHERE debtor_no=".db_escape($debtor_no)."";
		$result= db_query($sql,"Customer Branch Record Retreive");		
		$res =  db_fetch($result);
		return $res['cust_code'];
		
}

function get_cust_payment_bank_info_rep($trans_no)
{
	$sql = "SELECT mode_of_payment,cheque_no,dd_no,date_of_issue,dd_date_of_issue,pymt_ref 
...
function print_receipts()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j)
		{
			...
			if($ref_no>0){
				$result = get_allocatable_to_cust_transactions_refno($myrow['debtor_no'], $myrow['trans_no'], $myrow['type'],$ref_no);}
			else{
			...
			while ($myrow2=db_fetch($result))
			{
				...
				if($debtor_name != $myrow2['DebtorName'])
				{
					...
					$cust_code = get_customer_code($myrow2['debtor_no']);
					$rep->TextCol(0, 7,	$myrow2['DebtorName'].' - '.$cust_code);
					$rep->NewLine();
				}
           ...
           if (floatcmp($myrow['ov_amount'], 0))
			...
			if($ref_no >0)
				$words = no_to_words($tot_amount, ST_CUSTPAYMENT);
			else
...
{   
	...
	$no1 =round($number);
   $dec = explode('.', $number);
    $dec2 =isset($dec[1])?substr($dec[1],0,3):3;
