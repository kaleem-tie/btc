...
function print_customer_orders()
{
	...
	$cols = array(0, 50, 100, 150, 300,340,480,520,560,600,650,700,800,850);

	$headers = array(_('SO Date'), _('So Number'), 
	_('Cust PO Ref'),_('Customer Name'),_('Item Code'),_('Item Name'), _('Qty'),_('LC Rate'),_('Discount %'),_('LC Discount'),_('LC Value'),_('User'),_('Comments'));
	

	$aligns = array('left',	'left',	'left',	'left','left','left','right','right','right','right','right','center', 'left');
    ...
    $rep = new FrontReport(_('Sales Order Items Listing'), "SalesOrderItemsListing", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
  ...
  $result=get_transactions($fromcust,$from,$to, $dimension, $folk);
	...
	while ($myrow = db_fetch($result))
	{
		...
		$rep->AmountCol(7, 8, $myrow['unit_price'],3);
		$rep->TextCol(8, 9, $myrow['discount_percent']."%");
		$rep->AmountCol(9, 10, $myrow['unit_price']*($myrow['discount_percent'])*0.01,3);
		$rep->AmountCol(10, 11, $myrow['TotQuantity']*$myrow['unit_price']*(100-$myrow['discount_percent'])*0.01,3);
		$rep->TextCol(11, 12, get_transaction_user($myrow['order_no']));
		$rep->TextCol(12, 13,$myrow['comments']);	
