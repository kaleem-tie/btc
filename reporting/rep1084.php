...
function get_gl_trans_rep_eighty_four($type, $trans_id)
{
	$sql = "SELECT gl.*, cm.account_name, IFNULL(refs.reference, '') AS reference, user.real_name, 
			...
			IF(ISNULL(st.supp_reference), '', st.supp_reference) AS supp_reference, st.modified_supp_name
...
function print_bank_payments()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j)
		{
			...
			$rep->SetCommonData($myrow, null, $myrow, $myrow, ST_BANKPAYMENT_REP, $contacts);
			$rep->SetHeaderType('Header74');
			$rep->NewPage();
			$rep->NewLine();
			
			
			$result = get_gl_trans_rep_eighty_four(ST_BANKPAYMENT, $i);
			$total_amount = 0;
			$k=1;	
			while ($myrow2=db_fetch($result))
			{
				...
				$modified_supp_name =get_modified_supp_name(ST_BANKPAYMENT, $i, $myrow2["person_id"]);
				...
				elseif($myrow2["account"]==get_company_Pref('creditors_act')){
					...
					if($modified_supp_name!=''){
						...
						$rep->TextColLines(2, 3,	$supp_info['supp_name'], -2);
						$newrow = $rep->row;
				        $rep->row = $oldrow;
					}
			...
			//	$rep->TextCol(2, 3,	$supp_info['supp_name'], -2);
				...
				*/
				...
				$bank_cur_cod = get_bank_curr($myrow['bank_act']);
