...
function get_gl_trans_rep_eighty_one($type, $trans_id)
{
...
function print_bank_payments()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j)
		{
			...
			$rep->SetCommonData($myrow, null, $myrow, $myrow, ST_BANKTRANSFER, $contacts);
			$rep->SetHeaderType('Header29');
			$rep->NewPage();
			$rep->NewLine();
			
			
			$result = get_gl_trans_rep_eighty_one(ST_BANKTRANSFER, $i);
			$total_amount = 0;
			...
			$words = no_to_words($total_amount, ST_BANKTRANSFER);
