...
function get_transactions($to,$customer_id=null,$folk=0)
{
	...
	$sql = "SELECT type, reference, tran_date,invoice_type,bank_account,lpo_no,due_date,
	    IF(trans.prep_amount, trans.prep_amount, trans.ov_amount + trans.ov_gst + trans.ov_freight + trans.ov_freight_tax + trans.ov_discount + trans.ov_roundoff)
			AS TotalAmount,
		trans.alloc as settled_amt,trans.pdc_amt
...
function get_monthly_sales_invoice_info($customer_id,$folk,$start_date,$end_date)
{
	...
	$sql = "SELECT 
	    SUM(trans.ov_amount + trans.ov_gst + trans.ov_freight 
			+ trans.ov_freight_tax + trans.ov_discount + trans.ov_roundoff - (trans.alloc + trans.pdc_amt))	AS monthly_inv_amount
		FROM ".TB_PREF."debtor_trans trans
...
function get_monthly_unadjusted_credit_info($customer_id,$folk,$end_date)
{
	...
	$sql = "SELECT 
	    SUM(trans.ov_amount + trans.ov_gst + trans.ov_freight 
			+ trans.ov_freight_tax + trans.ov_discount + trans.ov_roundoff - (trans.pdc_amt+trans.alloc))	AS monthly_inv_amount
		FROM ".TB_PREF."debtor_trans trans
...
function print_statement_accounts_for_audit()
{
   ...
   $aligns = array('left',	'left',	'left',	'right', 'right','right', 'right', 'right');
    ...
    $rep = new FrontReport(_('Statement of Accounts with PDC Match'), 
	"StatementofAccountswithPDCMatch", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	while ($myrow = db_fetch($result))
	{
	   ...
	   if ($convert) $rate = get_exchange_rate_from_home_currency($myrow['curr_code'], $to);
		...
		if ($debtor != $myrow['name'])
		{
			...
			$rep->TextCol(6, 7,	"PDC Amount");
			$rep->TextCol(7, 8, "Inv.Balance");
		...
		$rep->NewLine();
		$inv_amt_total = $settled_amt_total = $pdc_amt_total = $inv_bal_total = 0;
		while ($trans = db_fetch($res))
		{
			...
			//$inv_balance = $trans['inv_amount']-$trans['settled_amt']-$trans['pdc_amt'];
			...
			$rep->AmountCol(6, 7, $trans['pdc_amt'], $dec);
			
			
			if (($trans['type'] == ST_JOURNAL && $inv_amount_dr) || $trans['type'] == ST_SALESINVOICE || $trans['type'] == ST_BANKPAYMENT)
				$inv_balance = $inv_amount_dr - $allocated_amt-$trans['pdc_amt'];
			else	
				$inv_balance = -$inv_amount_cr - $allocated_amt-$trans['pdc_amt'];
			
			
			if($inv_balance > 0.0){
			$rep->TextCol(7, 8, number_format2($inv_balance, $dec)." Dr");
			$inv_bal_total += $inv_balance;
			}
		    else{
			$rep->TextCol(7, 8, number_format2(-$inv_balance, $dec)." Cr");	
			$inv_bal_total -= -$inv_balance;
			}
		 ...
		 if($inv_amt_total > 0.0){
			$rep->TextCol(4, 5, number_format2($inv_amt_total, $dec)." Dr"); 
		 }
		 else{
			 $rep->TextCol(4, 5, number_format2(-$inv_amt_total, $dec)." Cr");
		 }	 
		 $rep->AmountCol(5, 6, $settled_amt_total, $dec);
		 $rep->AmountCol(6, 7, $pdc_amt_total, $dec);
	...
	$cur_net_balance = $cur_month_inv_value+$last_mn_inv_value+$last_2mn_inv_value+$last_3_mn_inv_value
	                   +$last_4_mn_inv_value-$unadjusted_cr_value;
	$rep->NewLine();
