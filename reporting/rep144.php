...
***********************************************************************/
$page_security = 'SA_RETAIL_CREDIT_SALES_REG_REP';
...
print_retail_credit_sales_register();
...
function get_sales_invoice_line_transactions($invoice_no, $foc_reg) {
...
function get_total_sales_value_and_discount_value($trans_no)
{
	...
	WHERE debtor_trans_type='10' AND debtor_trans_no=".db_escape($trans_no)." 
...
{
	$sql = "SELECT from_stk_loc,reference,ord_date FROM ".TB_PREF."sales_orders 
...
{
	 $sql="SELECT dt.reference,dt.tran_date from  ".TB_PREF."debtor_trans dt, ".TB_PREF."debtor_trans_details dtd where dt.type=dtd.debtor_trans_type and dt.trans_no=dtd.debtor_trans_no and dt.type=13 and dtd.debtor_trans_type=13 and dtd.id=".db_escape($doitem_no)."";
...
function print_retail_credit_sales_register()
{
	...
	$cols = array(0, 40, 75,100,210,230,280,330,380,430,475,515,555,600,650,700,750);
	
	$headers2 = array(_('Inv No.'), _('Inv Date'), _('Locn'), _('Cust Code - Name'),  _('Curncy'),
	_('SO Date'),_('SO No'),_('DO Date'),_('DO No'),
_('Gross Amt'), _('Disc Amt'), 
	_('Net Amt'), _('VAT Applied On'),
    ...
    $rep = new FrontReport(_('Retail Sales and Credit Sales Register'), 
	"RetailSalesandCreditSalesRegister", user_pagesize(), 7, $orientation);
	...
	while ($myrow = db_fetch($result))
	{
			...
			$rep->DateCol(5, 6,	$so_loc['ord_date'], true);
			$rep->TextCol(6, 7,	$so_loc['reference']);
			
			
			
				$inv_details = get_sales_invoice_line_transactions($myrow['trans_no'], $foc_reg);
			
			while ($inv=db_fetch($inv_details))
			{
			
			$do_details=get_do_info($inv['src_id']);
			
			
			$rep->DateCol(7, 8,	$do_details['tran_date'], true);
			$rep->TextCol(8, 9,	$do_details['reference']);
		...
		//    $rep->AmountCol(6, 7, $sales_disc_value['discount_value'],$dec);
			...
			$rep->AmountCol(9, 10, $sales_disc_value['sales_value'],$dec);
		    $rep->AmountCol(10, 11, $sales_disc_value['discount_value'],$dec);
			...
			$rep->AmountCol(11, 12, $net_amt,$dec);
			$rep->AmountCol(12, 13, $net_amt,$dec);
			$rep->AmountCol(13, 14, $vat_amt,$dec);
			$rep->AmountCol(14, 15, $roundoff_amt,$dec);
			$rep->AmountCol(15, 16, $net_amt+$vat_amt+$roundoff_amt,$dec);
		  ...
		  if (!$summaryOnly)
		  {
			...
			while ($inv=db_fetch($inv_details))
			{
				...
				$headers = array(_('Sl. No.'), _('Item Code'), _('Description'), _(''),  _('Units'),_(''),_(''),
				_(''),_(''),
	  _('Quantity'), _('FOC Qty'), _('Unit Rate'), _('Disc%'), _('Itm. Amount'),   _('VAT %'));
			    ...
			    $rep->TextCol(9, 10,	$DisplayQty, -2);
				$rep->TextCol(10, 11,	$DisplayFOCQty, -2);
			    $rep->TextCol(11, 12,	$DisplayPrice, -2);
				$rep->TextCol(12, 13,	$DisplayDiscAmt, -2);
			    $rep->TextCol(13, 14, $DisplayNet);
				
				$item = get_item($inv['stock_id']);
			    if($item['exempt']==0){
			    $rep->TextCol(14, 15, "5.00", -2);
				}
				else{
					$rep->TextCol(14, 15, "0", -2);
				}	
		...
		//	$rep->SetFont('helvetica', 'B', 9);
	        $rep->TextCol(11, 13, _('Item Total'));
	        $rep->AmountCol(13, 14, $inv_itm_total, $dec);
	     //   $rep->SetFont('', '', 0);
			$rep->Line($rep->row - 8);
			$rep->NewLine(2);
		 }
	...
	$rep->AmountCol(15, 16, $grand_net_incl_amt, $dec);
	//$rep->SetFont('', '', 0);
