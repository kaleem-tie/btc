...
function getTransactions($sales_person=0,$to,$leg_grp=0,$cust_class=0,$grp_comp,$area=0)
{
	...
	$sql = "SELECT debtor.name, debtor.curr_code,debtor.cust_code,
		...
		FROM ".TB_PREF."debtor_trans trans,
			...
			LEFT JOIN ".TB_PREF."cust_branch cb ON debtor.debtor_no=cb.debtor_no
	...
	if ($grp_comp != 0)
	{
		$sql .= " AND debtor.sale_cust_group_comp_id =".db_escape($grp_comp);
	}
...
function print_aged_salesman_analysis()
{
	...
	'right','right');
    	...
    	$params =   array( 	0 => $comments,
					...
					6 => array('text' => _('Sales Areas'), 'from' => $sarea, 		'to' => ''),
					7 => array('text' => _('Type'),		'from' => $summary,'to' => ''),
    				8 => array('text' => _('Currency'), 'from' => $currency, 'to' => ''),
				    9 => array('text' => _('Suppress Zeros'), 'from' => $nozeros, 'to' => ''));
	...
	$res = getTransactions($folk, $to, $leg_grp, $cust_class,$grp_comp,$area);
