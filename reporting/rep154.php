...
function GetSalesmanTrans($from, $to,$sales_person=0,$fromcust,$category=0,$subcategory=0)
{
	...
	$sql = "SELECT sm.salesman_name,line.stock_id,line.description,line.unit_price,
		...
		LEFT JOIN ".TB_PREF."stock_master s ON s.stock_id=line.stock_id
		LEFT JOIN ".TB_PREF."debtor_trans trans	ON trans.type = line.debtor_trans_type
	...
	if ($subcategory != 'all'){
		$sql .= " AND s.item_sub_category = ".db_escape($subcategory);
	}
    else if ($subcategory == 'all'){
		$sql .= " AND  (s.item_sub_category = 0  OR s.item_sub_category != 0)";
	}
...
function print_salesman_list()
{
    ...
    $params =   array( 	0 => $comments,
						...
						5 => array('text' => _('Sales Person'), 'from' => $sales_person_name, 'to' => ''),					 
    				    6 => array('text' => _('Location'), 'from' => $loc, 'to' => ''),
	    				7 => array(  'text' => _('Summary Only'),'from' => $sum,'to' => ''));
	...
	$result = GetSalesmanTrans($from, $to, $sales_person,$fromcust,$category,$subcategory);
