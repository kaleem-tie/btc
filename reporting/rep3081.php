...
function fetch_items($category, $subcategory, $item='')
{
...
function inventory_movements()
{
    ...
    $rep = new FrontReport(_('Stock Ledger Summary - 1'), "StockLedgerSummary1", user_pagesize(), 8, $orientation);
    if ($orientation == 'L')
	...
	$total_qty = $total_sal  = $total_tran = $total_open = $total_purch =0;
	
	    $grand_open = $grand_purch = $grand_tran = $grand_sal = $grand_adj = $grand_qty = 0 ;
	
	while ($myrow=db_fetch($result))
	{
		if ($catgor != $myrow['description'])
		{
	  //   if($total_qty != 0 || $total_sal != 0 || $total_tran != 0 || $total_open != 0 || $total_purch != 0 || $total_adj != 0 ){
				if ($catgor != '')
			{
				$rep->Line($rep->row  - 4);
	              $rep->NewLine(2);
				...
				$rep->AmountCol(3, 4, $total_open,$dec);
	            $rep->AmountCol(4, 5, $total_purch,$dec);
	            $rep->AmountCol(5, 6, $total_tran,$dec);
	            $rep->AmountCol(6, 7, $total_sal,$dec);
	            $rep->AmountCol(7, 8, $total_adj,$dec);
	            $rep->AmountCol(8, 9, $total_qty,$dec);
		...
		if ($qoh_start == 0 && $inward == 0 && $outward == 0 && $qoh_end == 0 && $sales_inward == 0 && $adj_inward == 0 && $transfer_inward == 0 && $purch_inward == 0)
			continue;
		$rep->NewLine();
		...
		$total_open += $qoh_start;
		$grand_open += $qoh_start;
	...
	$rep->AmountCol(3, 4, $grand_open,$dec);
	$rep->AmountCol(4, 5, $grand_purch,$dec);
	$rep->AmountCol(5, 6, $grand_tran,$dec);
	$rep->AmountCol(6, 7, $grand_sal,$dec);
	$rep->AmountCol(7, 8, $grand_adj,$dec);
	$rep->AmountCol(8, 9, $grand_qty,$dec);
	$rep->Line($rep->row  - 4);
	$rep->NewLine(2);
