...
***********************************************************************/
$page_security = 'SA_PRICE_LIST_SALE_REP';
// ----------------------------------------------------------------
// $ Revision:	2.0 $
// Creator:	Joe Hunt
// date_:	2005-05-19
...
function fetch_items($category=0, $fromsupp = ALL_TEXT)
{
...
function print_price_listing()
{
   ...
   // $showGP = $_POST['PARAM_5'];
	...
	$cols = array(0, 80, 360, 400, 450, 480, 520);

	$headers = array(_('Category/Items'), _('Description'),	_('UOM'), _('Price'),	
	_(''), _(''));
	...
	$result = fetch_items($category, $fromsupp);
	...
	while ($myrow=db_fetch($result))
	{
		...
		//$rep->AmountCol(4, 5, $item_info['material_cost'], $dec);
		/*if ($showGP)
		{
			...
			if ($price2 != 0.0)
				...
				$disp = 0.0;
			$rep->TextCol(5, 6,	number_format2($disp, user_percent_dec()) . " %");
		}*/
		if ($pictures)
		{
