...
function get_supp_pq($order_no)
{
   	$sql = "SELECT po.*, supplier.supp_name, supplier.supp_account_no,supplier.tax_included,
		...
		AND po.trans_type=".ST_PURCHQUOTE."
...
function get_pq_details($order_no)
{
	
	$sql = "SELECT poline.* 
		FROM ".TB_PREF."purch_order_details poline
		WHERE order_no =".db_escape($order_no)." AND poline.trans_type =".ST_PURCHQUOTE." ";
...
function print_po()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		if ($email == 1)
		{
			...
			$rep->title = _('PURCHASE QUOTATION');
			$rep->filename = "PurchaseQuotation" . $i . ".pdf";
		}	
		$rep->currency = $cur;
		$rep->Font();
		...
		$rep->SetCommonData($myrow, null, $myrow, $baccount, ST_PURCHQUOTE, $contacts);
		$rep->SetHeaderType('Header23');
		$rep->NewPage();

		$result = get_pq_details($i);
		$SubTotal = 0;
		$items = $prices = array();
		$k=1;
		while ($myrow2=db_fetch($result))
		{
		 /*	 $data = get_purchase_data($myrow['supplier_id'], $myrow2['item_code']);
			...
			$DisplayQty = number_format2($myrow2["quantity_ordered"],0);
		...
		if ($myrow['freight_cost'] != 0.0)
		{
			...
			$rep->TextCol(4, 7, _("Freight Charges"), -2);
		...
		if ($myrow['additional_charges'] != 0.0)
		{
			...
			$rep->TextCol(4, 7, _("Additional Charges"), -2);
			$rep->TextCol(7, 8,	$DisplayAdditionalCharges, -2);
			$rep->NewLine();
		}
		...
		if ($myrow['packing_charges'] != 0.0)
		{
			...
			$rep->TextCol(4, 7, _("Packing Charges"), -2);
		...
		if ($myrow['other_charges'] != 0.0)
		{
			...
			$rep->TextCol(4, 7, _("Other Charges"), -2);
			$rep->TextCol(7, 8,	$DisplayOther, -2);
			$rep->NewLine();
		}
		...
		$rep->Font('bold');
		$rep->TextCol(4, 7, _("TOTAL QUOTATION"), - 2);
		$rep->TextCol(7, 8,	$DisplayTotal, -2);
		$words = price_in_words($SubTotal, ST_PURCHQUOTE);
		if ($words != "")
		{
			$rep->NewLine(1);
