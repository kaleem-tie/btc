...
function getTransactions($supplier, $date)
{
	...
	$sql = "SELECT  supp_reference, tran_date, due_date, trans_no, type, rate,
			(ABS( ov_amount) + ABS( ov_gst)  -  ABS(ov_discount) + ABS (freight_cost)+ ABS(additional_charges)+ ABS(packing_charges)+ ABS(other_charges)+ ABS(freight_tax)+ ABS(additional_tax)+ ABS(packing_tax) + ABS(other_tax)  + ABS(ov_roundoff) -  alloc) AS Balance,
			(ABS( ov_amount) + ABS( ov_gst) -ABS(ov_discount) + ABS (freight_cost) + ABS(additional_charges)+ ABS(packing_charges)+ ABS(other_charges)+ ABS(freight_tax)+ ABS(additional_tax)+ ABS(packing_tax)+ ABS(other_tax) + ABS(ov_roundoff) ) AS TranTotal
		FROM ".TB_PREF."supp_trans
		WHERE  supplier_id = '$supplier'
		AND ROUND(ABS( ov_amount),$dec) + ROUND(ABS( ov_gst)-ABS(ov_discount)+ ABS(freight_cost)+ ABS(additional_charges)+ ABS(packing_charges)+ ABS(other_charges)+ ABS(freight_tax)+ ABS(additional_tax)+ ABS(packing_tax)+ ABS(other_tax) + ABS(ov_roundoff),$dec) - 
		ROUND( alloc,$dec) != 0
		AND  tran_date <='$date'
		ORDER BY  type,
			 trans_no";
...
function print_payment_report()
{
	...
	$sql = "SELECT supplier_id, supp_name AS name, curr_code, t.terms FROM ".TB_PREF."suppliers s, ".TB_PREF."payment_terms t
		WHERE  s.payment_terms = t.terms_indicator ";
	if ($fromsupp != ALL_TEXT)
