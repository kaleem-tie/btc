...
function print_po()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		$words = no_to_words($SubTotal + $myrow["freight_cost"]+ $myrow["additional_charges"] +$myrow["packing_charges"]+ $myrow["other_charges"], array( 'type' => ST_PURCHORDER, 'currency' => $myrow['curr_code']));
		...
		$rep->TextWrapLines($mcol+45,$mcol+425,$myrow['curr_code'].":".$words.$words1);
