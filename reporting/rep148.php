...
print_customerwise_delivery_orders_register();
...
function getTransactions($from, $to, $customer=0, $sales_person=0, $location)
{
	...
	$sql .= " GROUP BY trans.trans_no ORDER BY debtor.name,trans.trans_no";
...
function print_customerwise_delivery_orders_register()
{
    ...
    $rep = new FrontReport(_('Customer Wise DO Register'), 
	"CustomerWiseDORegister", user_pagesize(), 7, $orientation);
    ...
    $grand_net_incl_amt = $total_cust = 0;
	...
	while ($myrow = db_fetch($result))
	{
		  	...
		  	if ($customer_name != $myrow['name'])
		   {
			...
			if ($customer_name != '')
			{
				...
				$rep->TextCol(0, 6, _('Subtotal ').$customer_code ." - ".$customer_name);
		   ...
		   if ($cust_names != $myrow['name'])
		{
			$rep->NewLine();
			$rep->SetFont('helvetica', 'B', 9);
			$rep->TextCol(0, 6, $myrow['cust_code']." ".$myrow['name']);
			$rep->SetFont('', '', 0);
