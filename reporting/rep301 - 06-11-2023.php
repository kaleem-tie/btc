...
function getTransactions($category, $location, $date, $fromsupp = ALL_TEXT)
{
	$date = date2sql($date);
...
function print_inventory_valuation_report()
{
  ...
  {
	  $cols = array(0, 75, 225, 250,310,360,410, 470,515);

	$headers = array(_('Category'), '', _('UOM'), _('LOC-01'), _('LOC-02'), _('TOT-QTY'), _('Unit Cost'), _('Value'));
	...
	$res = getTransactions($category, $location, $date, $fromsupp);
	...
	while ($trans=db_fetch($res))
	{
		...
		if ($detail)
		{
			...
			$rep->AmountCol(5, 6, $trans['QtyOnHand'], get_qty_dec($trans['stock_id']));
