...
function get_customer_details_for_report($cr_sponsor_name='', $area=0, $folk=0)
{
	...
	{
		if($cr_sponsor_name != '')
		$sql .=" AND debtor.cr_no LIKE ".db_escape('%'.$cr_sponsor_name.'%')." or debtor.sponser_name LIKE ".db_escape('%'.$cr_sponsor_name.'%')."";
	}
...
function print_customer_details_listing()
{
	...
	if($cr_sponsor_name ==''){
		...
		$cr_name = "All CR No. / Sponsors";
	}else{
		$cr_name = " ".$cr_sponsor_name;
	}
	...
	$result = get_customer_details_for_report($cr_sponsor_name, $area, $folk);
	...
	while ($myrow=db_fetch($result))
	{
		...
		if ($printcustomer)
		{
			...
			$rep->TextCol(4, 5,	$myrow['cr_no']);
			$rep->TextCol(5, 6,	$myrow['sponser_name']);
			$rep->NewLine();
