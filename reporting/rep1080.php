...
$page_security = $_POST['PARAM_0'] == $_POST['PARAM_1'] ?
	'SA_BANKREP' : 'SA_BANK_DEPOSIT_REP';
// ----------------------------------------------------------------
// $ Revision:	2.0 $
// Creator:	Joe Hunt
// date_:	2005-05-19
...
function get_gl_trans_rep_eighty($type, $trans_id)
{
...
function print_bank_payments()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		if ($fno[0] == $tno[0])
			...
			$types = array(ST_BANKDEPOSIT, ST_SUPPAYMENT, ST_SUPPCREDIT);
		foreach ($types as $j)
		{
			
			
			$myrow = get_bank_trans_rep(ST_BANKDEPOSIT, $i);
			...
			$rep->SetCommonData($myrow, null, $myrow, $myrow, ST_BANKDEPOSIT, $contacts);
			$rep->SetHeaderType('Header28');
			$rep->NewPage();
			$rep->NewLine();
			
			
			$result = get_gl_trans_rep_eighty(ST_BANKDEPOSIT, $i);
			$total_amount = 0;
			...
			$doctype = ST_BANKDEPOSIT;
			$rep->NewLine();

			$memo = get_comments_string(ST_BANKDEPOSIT, $i);
			if ($memo != "")
			{
				$rep->NewLine();
			...
			$words = no_to_words(-$total_amount, ST_BANKDEPOSIT);
