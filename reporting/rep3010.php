...
function get_stock_qty($stock_id,$date,$loc_code)
{
		$to_date = date2sql($date);
	$sql = "SELECT sum(qty) from  ".TB_PREF."stock_moves 
			WHERE stock_id=".db_escape($stock_id)."
			AND tran_date <= '$to_date' AND loc_code=".db_escape($loc_code)."";
...
function getTransactions($category, $location, $date, $subcategory=0, $item='')
{
	$date = date2sql($date);
	...
	$sql = "SELECT item.category_id,
		...
		$sql .= " AND  (item.item_sub_category = 0  OR item.item_sub_category != 0)";
		}
		
		$sql .="AND item.units=units.abbr
...
function get_item_supplier_name_rep($supplier_id)
{
...
function print_inventory_valuation_report()
{
  ...
  if($location=='all') //ravi
  {
	  $cols = array(0, 50,230,280,350);

	$headers = array(_('Category'), _(''), _('UOM'), 
	 _('Quantity'));
    ...
    $rep = new FrontReport(_('Stock Valuation Report'), "StockValReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
	...
	$res = getTransactions($category, $location, $date, $subcategory, $item);
	...
	while ($trans=db_fetch($res))
	{
		...
		if ($detail)
		{
			...
			//$rep->TextCol(2, 3, get_item_supplier_name_rep($trans['supplier_id']));
			//$rep->TextCol(3, 4, $trans['supplier_item_code']);
			$rep->TextCol(2, 3, $trans['units']);
			
			//$rep->AmountCol(3, 4, get_stock_qty($trans['stock_id'],$date,'01'), get_qty_dec($trans['stock_id']));
			//$rep->AmountCol(4, 5, get_stock_qty($trans['stock_id'],$date,'02'), get_qty_dec($trans['stock_id']));
			
			$QtyOnHand = $trans['QtyOnHand'];
			$rep->AmountCol(3, 4, $QtyOnHand, get_qty_dec($trans['stock_id']));
			
			
			$dec2 = 0;
			price_decimal_format($UnitCost, $dec2);
			//$rep->AmountCol(4, 5, $UnitCost, 3);
			//$rep->AmountCol(5, 6, $ItemTotal, $dec);
			$rep->fontSize += 2;
		}
		
		$total += $QtyOnHand;
		$grandtotal += $QtyOnHand;
		//$total += $ItemTotal;
  ...
  {  
	...
	//$cols = array(0, 50,120,250,350,380,420,480,540);
	
	//$cols = array(0, 40,180,250,320,370,400);
      $cols = array(0, 50,230,280,320,420,480);

	$headers = array(_('Category'), '',   _('UOM'),
	_('Quantity'), _('Unit Cost'), _('Value'));
