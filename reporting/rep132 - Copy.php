...
function print_statement_accounts_balances()
{
	...
	"StatementofAccounts-BalanceConfirmation", user_pagesize(), 9, $orientation);
	...
	while ($myrow = db_fetch($result))
	{
	   ...
	   if ($convert) $rate = get_exchange_rate_from_home_currency($myrow['curr_code'], $to);
		...
		if ($debtor != $myrow['name'])
		{
			...
			$rep->NewLine(2);
			$rep->SetFont('helvetica', 'B', 9);
			$rep->Text($ccol+40, _('Balance due to us : ').$myrow['curr_code']." ".$custrec['Balance'].' (as per detailed statement attached)');
			$rep->SetFont('', '', 0);
			$rep->NewLine(3);
		    ...
		    $rep->NewLine();
			$rep->Text($ccol+40, $myrow['curr_code']." ".$custrec['Balance'].' due to them.');
			$rep->SetFont('', '', 0);
		    $rep->NewLine();
			$rep->NewLine(2);
