...
function getTaxTransactions($from, $to, $tax_id)
{
	...
	$sql = "SELECT d.debtor_no, d.name AS cust_name, d.tax_id, dt.type, dt.trans_no,  
			CASE WHEN dt.type=".ST_CUSTCREDIT." THEN (ov_amount+ov_freight+ov_discount+ov_roundoff)*-1 
			ELSE (ov_amount+ov_freight+ov_discount+ov_roundoff) END *dt.rate AS total
