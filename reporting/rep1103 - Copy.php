...
function get_pdc_transactions($from,$to,$customer_id=null)
{
   ...
   $aligns = array('left',	'left',	'right', 'right', 'right', 'right');
	...
	while ($myrow = db_fetch($result))
    {
       ...
       $rate = $convert ? get_exchange_rate_from_home_currency($myrow['curr_code'], Today()) : 1;
		...
		$pdc_res = get_pdc_transactions($from, $to, $myrow['debtor_no']);
        ...
        $rep->TextCol(5, 6, number_format2($invoice_total-$pdc_total, $dec)." Dr");
	...
	//$rep->TextCol(4, 5, number_format2($grand_pdc_total, $dec)." Cr");
    $rep->TextCol(5, 6, number_format2($grand_inv_total-$grand_pdc_total, $dec)." Dr");
