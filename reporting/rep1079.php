...
$page_security = $_POST['PARAM_0'] == $_POST['PARAM_1'] ?
	'SA_BANKREP' : 'SA_BANK_PAYMENT_REP';
// ----------------------------------------------------------------
// $ Revision:	2.0 $
// Creator:	Joe Hunt
// date_:	2005-05-19
...
function get_bank_trans_rep($type, $trans_no=null, $person_type_id=null, $person_id=null)
{
	...
	if($type != ST_BANKTRANSFER && $type != ST_SUPPAYMENT && $type!= ST_BANKPAYMENT)   // Ramesh added for salesman collection entry
		$sql .= " AND bt.amount>0 ";
...
function get_gl_trans_rep_seventy_nine($type, $trans_id)
{
...
function print_bank_payments()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j)
		{
			...
			$myrow = get_bank_trans_rep(ST_BANKPAYMENT, $i);
			...
			$rep->SetCommonData($myrow, null, $myrow, $myrow, ST_BANKPAYMENT, $contacts);
			$rep->SetHeaderType('Header27');
			$rep->NewPage();
			$rep->NewLine();
			
			
			$result = get_gl_trans_rep_seventy_nine(ST_BANKPAYMENT, $i);
			$total_amount = 0;
			while ($myrow2=db_fetch($result))
			{
				...
				if($myrow2["account"]==get_company_Pref('debtors_act')){
					$rep->TextCol(0, 1,	$cust_info['cust_code'], -2);
					$rep->TextCol(1, 2,	$cust_info['name'], -2);
				}	
				elseif($myrow2["account"]==get_company_Pref('creditors_act')){
					$rep->TextCol(0, 1,	$supp_info['supp_code'], -2);
					$rep->TextCol(1, 2,	$supp_info['supp_name'], -2);
				}
				elseif($myrow2["account"]!=get_company_Pref('debtors_act') && $myrow2["account"]!=get_company_Pref('creditors_act')){
				$rep->TextCol(0, 1,	$myrow2['account'].$counterparty_id, -2);
			    $rep->TextCol(1, 2,	$myrow2['account_name'] . ($counterpartyname ? ': '.$counterpartyname : ''), -2);
				}
				...
				$rep->row = $oldrow;
				*/
				$rep->AmountCol(2, 3, $myrow2['amount'], $dec, -2);
			...
			$memo = get_comments_string(ST_BANKPAYMENT, $i);
			if ($memo != "")
			{
				$rep->NewLine();
			...
			$rep->Text($ccol+400, _("Total Amount  "));
			$rep->Text($ccol+490, _(":  "));
			...
			$words = no_to_words($total_amount, ST_BANKPAYMENT);
