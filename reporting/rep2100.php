...
function print_remittances()
{
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j)
		{
			...
			$rep->SetCommonData($myrow, null, $myrow, $baccount, ST_SUPPAYMENT_REP, $contacts);
			$rep->SetHeaderType('Header26');
			$rep->NewPage();
			...
			while ($myrow2=db_fetch($result))
			{
				...
				{	
                  /* 	$rep->TextCol(0, 1,	$supp_details['supp_code'], -2);	
                   ...
                   $rep->TextCol(0, 1,	$supp_details['supp_code'], -2);	
				   // $rep->TextCol(0, 1,	$myrow2['account'].' '.sprintf(' %05d', $myrow2["person_id"]), -2);	
					//$rep->TextCol(1, 2,	get_subaccount_name($myrow2["account"], $myrow2["person_id"]), -2);			
                    $rep->TextCol(1, 2,	$supp_details['name'], -2);						
				}
				else
				{	
			...
			$words = no_to_words($debit, ST_SUPPAYMENT);
