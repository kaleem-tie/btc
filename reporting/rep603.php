...
function get_bank_transactions($from, $to, $account)
{
	$from = date2sql($from);
	$to = date2sql($to);
	$sql = "SELECT sum(amount) as amount,type,trans_date FROM ".TB_PREF."bank_trans
		WHERE bank_act = '$account'
		AND trans_date >= '$from'
		AND trans_date <= '$to'
		AND amount <> 0
		group by type,trans_date 
		ORDER BY trans_date";
...
function print_bank_transactions()
{
	...
	$rep = new FrontReport(_('Bank Statement Day wise'), "BankStatement", user_pagesize(), 9, $orientation);
	$dec = user_price_dec();

	$cols = array(0, 150, 220, 300, 380, 440,515);

	$aligns = array('left',	'left','left',	'right', 'right', 'right');

	$headers = array(_('Type'),	 _(''), _('Date'), _('Debit'),	_('Credit'), _('Balance'));
	...
	while ($account=db_fetch($result))
	{
		...
		if ($prev_balance != 0.0 || $rows != 0)
		{
			$rep->Font('bold');
			$rep->TextCol(0, 2,	$act);
			$rep->TextCol(2, 3, _('Opening Balance'));
			if ($prev_balance > 0.0)
				$rep->AmountCol(3, 4, abs($prev_balance), $dec);
			else
				$rep->AmountCol(4, 5, abs($prev_balance), $dec);
			$rep->Font();
			$total = $prev_balance;
			$rep->NewLine(2);
			...
			if ($rows > 0)
			{
				...
				while ($myrow=db_fetch($trans))
				{
					...
					$rep->DateCol(2, 3,	$myrow["trans_date"], true);
					if ($myrow['amount'] > 0.0)
					{
			...
			$rep->TextCol(1, 3, _("Total Debit / Credit"));
			$rep->AmountCol(3, 4, $total_debit, $dec);
			$rep->AmountCol(4, 5, $total_credit, $dec);
			$rep->NewLine(2);

			$rep->Font('bold');
			$rep->TextCol(1, 3,	_("Ending Balance"));
			if ($total > 0.0)
				$rep->AmountCol(3, 4, abs($total), $dec);
			else
				$rep->AmountCol(4, 5, abs($total), $dec);
			$rep->Font();
			...
			$rep->TextCol(1, 2, _("Net Change"));
			if ($total > 0.0)
				$rep->AmountCol(3, 4, $net_change, $dec, 0, 0, 0, 0, null, 1, True);
			else
				$rep->AmountCol(4, 5, $net_change, $dec, 0, 0, 0, 0, null, 1, True);
		}
	}
	$rep->End();
}
