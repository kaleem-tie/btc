...
***********************************************************************/
$page_security = 'SA_PENDING_DELIVERY_REP';
...
function get_transactions($customer_id=null,$date_)
{
   ...
   $sql = "SELECT 
		...
		$sql .=  " AND sorder.delivery_date < '$date_'";		
	...
	$sql .= " AND soplan.planned_date < '$date_'";		
	if ($customer_id != ALL_TEXT)
			$sql .= " AND sorder.debtor_no = ".db_escape($customer_id);	
...
function print_sales_deliveries()
{
	...
	$cols = array(0, 30,70,130,180,225,270,380,420,500,550);

    $headers = array(_('Reference'), _('Order Date'), _('Sales person'), 
                 _('Dispatch Location'), _('Delivery Date'), _('Address'), 
	            _('Item Name'), _('Quantity'),  _('Customer'),_('Contact Number'));
				
    $aligns = array('left','left','left','left','left','left','left','center',
	'left','right');

    $params =   array( 	0 => $comments,
	                    1 => array('text' => _('End Date'), 'from' => $end_date,'to' => ''),
    				    2 => array('text' => _('Customer'), 'from' => $cust,   	'to' => ''),
						3 => array('text' => _('Plan Type'), 'from' => $plan,   	'to' => ''));

    $rep = new FrontReport(_('Delayed Deliveries'), "Delayed Deliveries", user_pagesize(), 7, $orientation);
  ...
  $result=get_transactions($fromcust,$end_date);
  $k=1;
  $tot_value = 0;
	...
	while ($myrow = db_fetch($result))
	{
		...
		if ($destination){
		$rep->TextCol(5, 6, $myrow['delivery_address']);	
		}
         
		else{
		$oldrow = $rep->row;
		$rep->TextColLines(5, 6, $myrow['delivery_address'], -2);
		$newrow = $rep->row;
		$rep->row = $oldrow;
		}
		...
		$rep->TextCol(7, 8, $myrow['TotQuantity']-$myrow['qty_sent']);
		$name = $myrow["debtor_no"]." ".$myrow["name"];
		$rep->TextCol(8, 9, $name);
		$rep->TextCol(9, 10, $myrow['contact_phone']);
