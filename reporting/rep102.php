...
function print_aged_customer_analysis()
{
	...
	$sql = "SELECT m.debtor_no, m.name, m.curr_code FROM ".TB_PREF."debtors_master m, ".TB_PREF."cust_branch cb WHERE m.debtor_no=cb.debtor_no ";
	...
	if ($folk != 0 )
    {
        $sql .= " AND cb.salesman=".db_escape($folk);
    }
	$sql .= " GROUP BY m.debtor_no ORDER BY m.name";
