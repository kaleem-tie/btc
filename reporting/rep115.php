...
//ravi
function get_outstanding_balance($debtorno, $to)
{
	if($to)
		$to = date2sql($to);
	$sql = "SELECT SUM(IF(t.type = ".ST_SALESINVOICE." OR (t.type IN (".ST_JOURNAL." , ".ST_BANKPAYMENT.") AND t.ov_amount>0),
...
function get_transactions($debtorno, $from, $to)
{
    ...
    $sign = "IF(trans.type IN(".implode(',',  array(ST_CUSTCREDIT,ST_CUSTPAYMENT,ST_BANKDEPOSIT,ST_CUSTPDC))."), -1, 1)";
    ...
    $sql = "SELECT trans.*, comments.memo_,
        $sign*IF(trans.prep_amount, trans.prep_amount, trans.ov_amount + trans.ov_gst + trans.ov_freight + trans.ov_freight_tax + trans.ov_discount + trans.ov_roundoff)
            AS TotalAmount,
...
function print_customer_balances()
{
    ...
    $folk = $_POST['PARAM_4'];  // added by Faisal to filter by sales folk
    ...
    $cols = array(0, 50, 130, 190, 250, 320, 385, 450, 515);

    $headers = array(_('cust code'), _('Name'), '', _('Open Balance'), _('Debit'), _('Credit'), '', _('Balance'));
    ...
    $sql = "SELECT ".TB_PREF."debtors_master.debtor_no, name, curr_code, cust_code FROM ".TB_PREF."debtors_master
    ...
    while ($myrow = db_fetch($result))
    {
		...
		//ravi
		$cust_outstanding_balance=get_outstanding_balance($myrow['debtor_no'], $to);
		$outstanding_db = $cust_outstanding_balance ? round2(abs($cust_outstanding_balance['charges'] * $rate), $dec) : 0; // db
        $outstanding_cr = $cust_outstanding_balance ? round2(abs($cust_outstanding_balance['credits'] * $rate), $dec) : 0; // cr
